...
- `export_attributes=True` — клеточные атрибуты рампы и пола уезжают каналами
  `_RAMP` и `_NOFLOOR` (`CONVENTIONS.md`, «Terrain-объект»). Без неё экспортёр
  пропускает пользовательские атрибуты, и импорт прочитал бы сетку без единой
  рампы (BLND-9); отсутствие настройки в версии Blender — предупреждение, а не
  отказ, потому что сцена без рамп и провалов ими не пользуется.
