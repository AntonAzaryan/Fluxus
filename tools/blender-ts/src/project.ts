...
import type { SpatialLayerContext } from './layer.js';
...
export async function openImportTarget(
  ...
  return {
    ...
    initialPath,
    decorationsPath,
    context: contextOf(session, target, input.binding),
    // Группа записи — ровно те документы, которые пишет импорт (ED-21):
    // производные данные лежат в двух документах, и уйти на диск они обязаны
    // одной записью, иначе на диске окажется половина слоя. Манифест в группу не
    // входит: импорт его не трогает (BLND-2), а держать вместе документы,
    // которых правка не касается, значило бы отвергать сохранение из-за чужих
    // несохранённых правок.
    group: { id: sceneId, members: [sceneId, presentationId] },
  };
}
