 ...
 *
 * Набор, который редактор везёт с собой, покрывает числовые типы полей схем ядра
 ...
 *
 * Поле-ссылка (`entity`, `ecs-foundation` ECS-6) своего редактора пока не имеет и
 * показывается сырым контролом инспектора: осмысленный контрол для него — пикер
 * сущности сцены, то есть отдельная работа, а не строка в этом файле.
 *
