 ...
 * оболочкой, что у игрового клиента (`client-shell` SHELL-1) — ядро в
...
export function startPreviewSimulation(
  ...
  if (scene.terrain !== undefined) {
    ...
    //
    // `terrain` в зависимостях — ТОТ ЖЕ запрос, что уходит на `Simulation` ниже
    // (PHYS-14 требует одного источника уровней у обоих гейтов). Системе он
    // сейчас не нужен — она берёт запрос из контекста тика, — но объект
    // зависимостей заполняется целиком: он же пойдёт в `createPhysicsApi`, если
    // превью обзаведётся лучом, и половинчатый дал бы луч, считающий уровень
    // нулём там, где гейт пары видит реальный террейн.
...
export interface ScenePreviewHooks {
  ...
  /**
   * Вход в превью, до первого тика. Незакрытого взаимодействия к этому моменту
   * уже нет, и снимает его не вклад: приостановку авторинга каркас берёт раньше
   * запуска, а открытую транзакцию отменяет сама приостановка (ED-9, ED-18) —
   * «превью MUST NOT записывать что-либо в документы» не может зависеть от
   * того, вспомнил ли вклад про открытый мазок.
   *
   * Области остаётся её собственное состояние взаимодействия: признак
   * перетаскивания и начатый мазок живут в инструментах, а не в сессии, и о
   * чужой приостановке не знают.
   */
  enter(): void;
  /**
...
export function createScenePreview(hooks: ScenePreviewHooks): PreviewSource {
  return {
    ...
    start(): PreviewRun {
      ...
      try {
        // Вход объявляется до запуска: с этого момента кадр наполняет прогон, а
        // не документы (REND-11). Запрет авторинга (ED-9) к этому моменту уже
        // взят каркасом — вклад его не берёт и не снимает.
        hooks.enter();
