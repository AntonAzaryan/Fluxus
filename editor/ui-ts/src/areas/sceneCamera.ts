...
export interface SceneCameraOptions {
  ...
  /**
   * Секция конфига камеры манифеста визуалов (`assets` ASSET-10) — те же числа,
   * с которыми камеру поднимает игровой клиент.
   *
   * ED-13 не оставляет выбора: «Превью игрового кадра SHALL идти с тем же
   * конфигом камеры, что и игровой клиент: наклон, дистанция и FOV по умолчанию
   * совпадают, и автор видит композицию такой, какой её увидит игрок». Адрес у
   * этих чисел один (ASSET-10), и не пойти за ним значило бы держать в
   * редакторе второй набор умолчаний камеры — расходящийся с игрой молча,
   * потому что сегодня он с ней случайно совпадает.
   *
   * Ложится ПОД редакторские отклонения: потолок зума вьюпорта — настройка
   * конвейера редактора (`EDITOR_CAMERA_CONFIG`, CAM-1), и манифест её не
   * отменяет. В перечне ED-13 потолка зума нет, а обзор арены без него
   * упирается в кламп (CAM-8).
   */
  readonly manifest?: Partial<CameraConfig>;
  /**
   * Настройки конвейера (CAM-1) поверх манифеста и редакторских
   * (`EDITOR_CAMERA_CONFIG`): поданное здесь перекрывает их, всё остальное
   * остаётся игровым умолчанием.
   */
...
export function createSceneCamera(options: SceneCameraOptions): SceneCamera {
  ...
  const rig = new CameraRig({
    ...
    // Слоёв три, и порядок их — правило, а не вкус: игровые умолчания кода
    // (их кладёт сам `CameraRig`), поверх — числа манифеста визуалов (ASSET-10,
    // ED-13: кадр автора и кадр игрока совпадают), поверх — редакторские
    // отклонения конвейера (CAM-1), и последним — поданное сборкой.
    config: { ...options.manifest, ...EDITOR_CAMERA_CONFIG, ...options.config },
  });
