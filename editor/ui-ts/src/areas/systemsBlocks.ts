 ...
 *
 * Чего здесь нет: проверки арности и типов значений. SYS-3 прямо выносит их в
 * ошибку времени вычисления, и ответ на это — не второй проверяльщик, а превью
 * (ED-9).
 *
 ...
 * рассудит вычисление, а не редактор (SYS-3).
 */
...
export function expressionBlock(env: BlockEnvironment, path: JsonPath, value: JsonValue): UiNode {
  ...
  if (block !== undefined && !block.variadic) {
    ...
    // Форму аргументов ядро не структурирует: список n-арный, и автор
    // добавляет и убирает выражения сам (арность — ошибка времени вычисления).
