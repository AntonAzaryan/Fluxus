...
export interface StageDraft {
   ...
   * Автор видит ту же яркость и то же свечение, что игрок (`editor` ED-1).
   */
   ...
   * что игрок (`editor` ED-1), и правка кривизны под водоёмом меняет её берег не позже
...
export function createSceneStage(options: SceneStageOptions): SceneStage {
  ...
  // игрового клиента. Тождество кадров (`editor` ED-1) выполняется этим по построению,
   ...
   * нет, он приходит вторым запросом (ASSET-14). Подписка одна на все
   * подсистемы — см. `stopAssetWatch` ниже, где к частицам добавлены модели.
   */
  ...
  const draw = (now: number, dt: number): void => {
    try {
      ...
      // совпадают по построению (`editor` ED-1).
