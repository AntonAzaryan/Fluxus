 ...
 *
 * ## Второй потребитель канала изменений: hot-reload конвейера Blender
 *
 * Канал внешних изменений заводился ради ED-12 — «реакция на изменение дерева
 * извне» — и оказался ровно тем, что нужно циклу `blender-pipeline` BLND-12:
 * watch-режим импорта (`tools/blender-ts/src/watch.ts`) пишет документы,
 * наблюдатель сервера видит запись, вкладка перечитывает изменившийся документ
 * (`documentRefresh.ts`) и переподаёт его кадру существующими каналами
 * (REND-11, REND-14, REND-17, REND-18, CAM-7). Своего сокета инструмент
 * авторинга поэтому не заводит: «файл изменился» рассказывает тот, у кого
 * дерево есть, — и одному каналу не нужно второго (design, решение 12).
 */
