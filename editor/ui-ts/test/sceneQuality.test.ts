...
describe('вьюпорт редактора живёт на «ультре» (QUAL-1, design D4)', () => {
  ...
  it('реестр кадра правки — ручки его подсистем; тумана в нём нет (REND-16)', () => {
    ...
    expect(controller.knobs.map((knob) => knob.name).sort()).toEqual([
      // Потолок дробления наземных фигур (REND-43): подсистема эффектов
      // объявляет его наравне с прочими, и вьюпорт, живущий на «ультре», его не
      // ставит — фигура облегает рельеф так, как её написал автор.
      'effects.shapeDetail',
      ...
      // редактора локальный свет не потребовал ни одной (`editor` ED-1).
      ...
      // у сглаживания умолчание прямое — четыре сэмпла цели сцены.
      'postprocess.antialias',
      'postprocess.bloom',
      'postprocess.bloomResolution',
      'postprocess.lut',
      'terrain.curvatureTessellation',
      // Потолок слотов текстурирования (REND-39) приходит той же подсистемой
      // террейна; вьюпорт живёт на «ультре», и потолка он не ставит.
