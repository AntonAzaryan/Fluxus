...
describe('ED-22: наложения внутри кадра требованием не запрещены', () => {
  it('кадр на странице есть и не несёт ни одного класса, кроме своего', () => {
    // Не «нет класса области токенов»: кадр лежит внутри неё, он потомок корня
    // приложения. Держит кадр то, что правила хрома красят по классу хрома, а
    // взять его кадру неоткуда — своих классов у него ровно один.
    expect(frames.length).toBeGreaterThan(0);
    for (const frame of frames) {
      expect(frame.classes).toEqual([VIEWPORT_CLASS]);
      expect(frame.classes).not.toContain(TOKEN_SCOPE_CLASS);
    }
  });

  it('кадр не несёт собственных объявлений стиля', () => {
    // Строка стиля обошла бы и сброс токенов, и всю таблицу разом; узел её
    // и не умеет нести (`dom/node.ts`), но кадр — то место, где это важнее
    // всего, поэтому проверено отдельно.
