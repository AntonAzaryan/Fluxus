 ...
 *   «диапазон», «ссылка на существующее», «должно быть/не быть», «уходит на
 *   диск вместе с» и, отдельным случаем, «валидатор отверг». Последний нужен
 *   честно: часть проверок ядра
...
/**
 * Правки перечисленных документов обязаны уйти на диск одной записью (ED-21).
 *
 * Единственное ожидание, говорящее не о значении по пути: нарушено не
 * содержимое документа, а состав записи, и чинится оно составом — потребитель
 * досохраняет названных, а не правит найденное место. Ожидание существует
 * потому, что документ, формат которого ещё не нормирован (парный
 * presentation-документ ED-19), проверяем только так.
 */
export interface ExpectedTogether {
  readonly kind: 'together';
  /** Все документы группы, включая тот, на котором стоит находка. */
 ...
 *
 * Этим и живёт сохранение (ED-21): состояние дерева «после записи» — такой же
 * источник, и согласованность на диске проверяют те же правила-вклады, что
 * подсвечивают её при правке. Второй формы правила и второй формы находки для
 * сохранения не существует — она была бы вторым описанием, запрещённым ED-30.
 */
