...
/**
 * Чем правило объявляет своё пространство причин: id и коды, которые оно
 * способно сообщить. Из пары выводятся ключи ресурсов (`reasons.ts`), и обе
 * стороны отчёта ED-28 считаются по ней же.
 *
 * Отдельная форма, а не только поле правила, потому что спрашивают её не
 * прогоном: гейту бандла и отчёту нужны id и коды, а не `check` — правило фазы
 * записи (ED-21) строится по составу сохранения и до прогона не существует
 * вовсе, тогда как строки его причин обязаны лежать в бандле всегда.
 */
export interface RuleReasons {
  readonly id: string;
  /**
   * Коды причин правила. Объявление обязательно: правило без него не завело бы
   * ни одной строки в бандле, и его находка показалась бы автору ключом — тем
   * самым невидимым отсутствием ресурса, которое ED-28 требует делать видимым.
   * Разойтись с сообщаемым объявление не может: код, которого здесь нет, прогон
   * не принимает (`runner.ts`).
   */
 ...
 */
export interface ValidationRule extends ValidationRuleContribution, RuleReasons {
