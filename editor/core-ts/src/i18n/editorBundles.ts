 ...
 * операторы, ED-4, ED-5), поля форматов документов ядра (SER-5), которые
 * показывает инспектор (ED-24), описания операций авторинга (ED-29) и
 * пространство слоя валидации — описание каждого правила и причина каждой его
 * находки (`validation.*`). Строк хрома интерфейса здесь нет — их приносит тот,
 * кто приносит сам интерфейс; выдуманная подпись несуществующей кнопки ничем не
 * лучше отсутствующей.
 *
 * Причины валидации лежат именно здесь, а не у интерфейса, по трём совпадающим
 * основаниям: ключ минтят правила этого пакета (ED-8), тот же ключ читает
 * машинный потребитель, которому интерфейса не видно вовсе (ED-30), и описание
 * правила из этого же бандла служит описанием в каталоге — «один и тот же
 * текст» ED-30. Строка, заведённая у потребителя, была бы вторым набором
 * формулировок и разошлась бы с правилом при первом же переименовании.
 */
