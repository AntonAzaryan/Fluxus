 ...
 *
 * Порядок ключей — порядок конвенции (`CONVENTION_SLOTS`), затем имена вне
 * конвенции в том порядке, в каком их подали: сборщик их пропускает, а вердикт
 * выносит ядро — имя вне конвенции оно отвергает на регистрации (SYS-3), и
 * второй проверки того же самого в редакторе быть не должно (ED-1). Не порядок заполнения: два
 ...
 * (такой узел ядро на регистрации отвергнет, и порядок ему уже безразличен).
 *
