 ...
 * адрес в файл `--address-file` и отмечает каждый свой запуск строкой в
 * `--mark`.
 *
 ...
 * читает. Отметка запуска нужна проверке «второго процесса не появилось»: по
 * записи контейнера этого не разглядеть — она перезаписывается.
 */
