 ...
 *
 * Срок ВЫВЕДЕН, а не выбран красивым числом: приложение, чья политика завершения
 * останавливает свой сервис (`server-manager` MGR-4), успевает ответить только
 * если срок покрывает обе милости на этом пути —
 *
 *   3000 мс  штатный выход процесса сервера матча после SIGTERM
 *            (`game/server-agent-ts/src/stand/process.ts`, GRACE_MS)
 * + 2000 мс  штатный выход самого сервиса после SIGTERM
 *            (`host/service.ts`, GRACE_MS)
 * + 3000 мс  запас на дорогу: кадр по управляющему каналу туда и обратно, шаг
 *            IPC до страницы и обратно, SIGKILL после каждой из милостей
 * = 8000 мс
 *
 * Числами, а не импортом: первое из слагаемых живёт в пакете ИГРЫ, и контейнер,
 * который его импортирует, знает, что за сервис он поднял, — ровно то, что ему
 * закрыто (DSK-2, DSK-3). Поэтому вывод записан здесь словами и обязан
 * пересматриваться вместе с теми константами.
 *
 * Срок остаётся крайним, а не ожиданием: окно, которое нельзя закрыть из-за
 * молчащей страницы, было бы хуже неостановленного сервиса.
 */
const CLOSE_ANSWER_MS = 8000;
