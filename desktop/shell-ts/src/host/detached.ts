...
export interface DetachedFiles {
   ...
   *
   * Лежит рядом с адресным файлом, потому что у ОТВЯЗЫВАЕМОГО сервиса живёт
   * ровно столько же: закрепление обязано находиться и через границу сессий —
   * новая сессия, нашедшая пережившего, читает его отсюда же (DSK-8, сценарий
   * «Сервис пережил сессию»). Обычному сервису та же раскладка достаётся через
   * `servicePinFile`, а через границу сессий его закрепление не действует.
   */
...
/**
 * Путь файла закрепления ЛЮБОГО объявленного сервиса (DSK-8), а не только
 * отвязываемого: оговорки «только отвязываемый» в требовании нет — «объявленный
 * профилем сервис (DSK-7), чей канал существует только шифрованным с self-signed
 * сертификатом, SHALL иметь способ назвать контейнеру закрепление своего
 * сертификата».
 *
 * Раскладка та же, что у отвязываемого, и это не совпадение: файл пишет САМ
 * процесс по подстановке `{pinFile}`, и путь у него один — где бы его потом ни
 * читали. Отвязываемость меняет не наличие файла, а СРОК действия закрепления:
 * закрепление пережившего сессию сервиса читает и новая сессия, а закрепление
 * обычного живёт ровно столько, сколько поднявший его процесс (`certificatePins`).
 */
export function servicePinFile(stateDir: string, id: BridgeServiceId): string {
  return detachedFiles(stateDir, id).pinFile;
}

/**
 * Убирает закрепление сервиса, не трогая ни адреса, ни pid (DSK-8): оставленный
 * файл продолжал бы расширять доверие на сертификат процесса, которого больше
 * нет. У отвязываемого закрепление уносит `forgetDetached` — вместе со всем
 * остальным, что тот пережил.
 */
export function forgetPin(pinFile: string): void {
