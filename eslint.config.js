...
export default defineConfig([
  ...
  globalIgnores([
    ...
    'content/**',
    'openspec/**',
    // Артефакты отладочных прогонов (`npm run demo:debug`, CLI-11): трасса, запись,
    // журнал и отчёт — данные, а не код; случайный скрипт под `runs/` иначе
    // краснил бы линт всего репозитория.
    'runs/**',
  ...
  /**
   * `no-non-null-assertion` выключен ровно в пяти пакетах — тех, чьё хранилище
   * это типизированные массивы, и где `!` не небрежность, а идиома.
   *
   * Под `noUncheckedIndexedAccess` чтение `arr[i]` имеет тип `T | undefined`
   * даже когда индекс ограничен строкой выше, и `!` — способ сказать «инвариант
   * держится здесь», не платя за это ветвлением. Платить пришлось бы в
   * потиковом и покадровом пути: SoA-хранилище ECS (`core-ts`), декодированные
   * буферы моделей и текстур (`assets-ts`), вершинные и инстансные буферы
   * (`render-ts`), параллельные массивы по слотам матча (`net-ts`), плотные
   * сетки клеток импортёра (`tools/blender-ts`). Замена на `?? fallback` там не
   * упрощение, а молчаливая смена данных.
   *
   * Во всех остальных пакетах правило включено: `!` там не идиома, а сигнал.
   * Немногие законные места закрыты точечными директивами с причиной — их
   * держит честными `--report-unused-disable-directives`.
   *
   * Счётчик и методика замера — в `LINT_BASELINE.md`.
   */
  {
    files: [
      'engine/core-ts/**/*.ts',
   ...
   * не утверждает, кроме порядка величины). Сторожа памяти
   * (`performance-budget` PERF-10, PERF-11) — тот же случай и то же основание:
   * печать при КАЖДОМ прогоне для них нормативна, а ассертят они только порядок
   * величины.
   */
  {
    files: [
