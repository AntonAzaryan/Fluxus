...
export interface MatchConfig {
   ...
   * им команда обязана совпасть с командой сущности этого слота, ЕСЛИ та есть в
   * мире на сборке: расхождение — отказ сборки матча, а не молча действующая
   * чужая точка зрения. Снапшот, отобранный по чужой команде, отнял бы у игрока
   * его собственную сущность (NET-15) и выдал бы ему чужую видимость. Умолчания
   * «всё видно» здесь быть не может — это wallhack по невнимательности.
   *
   * Сущности, которой на сборке ещё нет, сверять не с чем: объявленное значение
   * действует как названо, и соответствие будущей расстановке остаётся на авторе
   * конфига — NTR-9 называет эту оговорку прямо, а не умалчивает.
   */
   ...
   *
   * Эта же граница служит потолком адаптивного запаса разметки `lead` (NTR-7):
   * кадр за окном сервер отбрасывает, и рост запаса выше окна означал бы ввод,
   * теряемый уже не каналом, а собственной разметкой клиента. Поэтому число
   * отвечает и на второй вопрос — какой круг матч берётся вытянуть.
   */
 ...
 *
 * Сверить можно только то, что в мире уже есть: слот, сущность которого
 * появляется ПОЗЖЕ сборки, принимает объявленную команду как названа (NTR-9).
 * Второй сверки на живом мире здесь не заводится намеренно — точка зрения
 * считается один раз и свойством тика быть не должна.
 *
...
function resolveRates(config: MatchConfig): MatchRates {
  ...
  // разметку: клиент помечает кадр тиком `serverTick + lead`, и нижняя граница
  // этого запаса — `inputDelay` из того же конфига, который прислал сервер
  // (NTR-7). Верхняя граница запаса — само окно, поэтому проверка сторожит и
  // потолок адаптации, а не только стартовое значение.
...
export class MatchServer {
  ...
  unbar(slot: number): void {
    ...
    // Исход ПРОШЛОЙ аренды перестаёт быть «запрет»: аренда владельца кончилась
    // разрывом канала, и запрета больше нет. Оставь его — и обвязка (SRV-4) не
    // отличила бы «всё ещё заперт» от «запрет снят, владелец ещё не вернулся»:
    // оба читались бы как `removed`, а кнопка админа так и осталась бы
    // «вернуть» для слота, который уже свободен.
    if (this.slotEnd[slot] === 'barred') this.slotEnd[slot] = 'closed';
  }
  ...
  private greet(
    ...
    //
    // Отказ идёт ВСЯКОМУ входу в запертый слот, а не только заявившему роль
    // владельца. `role` — поле провода, которое пишет сам подключающийся
    // (`Hello`), и роль в нём не удостоверена ничем: запрет, снимаемый строкой
    // `"substitute"` в собственном сообщении запрещённого, не запрет. Слот
    // владельца при этом опознаётся его же ростерным идентификатором, так что
    // «зайти заместителем» — это тот же запрещённый вход, назвавшийся иначе.
    //
    // Заместителя (NTR-18) запирание по-прежнему не трогает: УЖЕ СИДЯЩИЙ
    // заместитель слот ведёт дальше — `bar` его не рвёт (см. ниже), — а при его
    // отсутствии слот получает predicted-кадры (NTR-7), ровно как сказано в
    // NTR-19. Права ЗАНЯТЬ запертый слот заново требование не даёт никому.
    if (this.slotBarred[slot] === true) {
  ...
  private sendPauseState(connection: Connection): void {
    ...
    if (this.pausePhase !== 'running') {
      ...
      // Снапшот СЧИТАЕТСЯ, как и всякий другой (NTR-11): счётчик сервера и сумма
      // счётчиков соединений у хоста иначе расходятся на один за каждый
      // реконнект в заморозку — и админ, читающий их рядом (SRV-4), видел бы
