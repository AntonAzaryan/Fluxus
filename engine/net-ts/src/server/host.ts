...
export class MatchHost {
  ...
  private sendOutgoing(outgoing: Outgoing, transport: Transport): number {
    ...
    if (isSnapshot && this.skipsSnapshot(transport, counted)) {
      ...
      // Сервер же посчитал его отправленным, когда положил в исходящие, а
      // NTR-11 считает ОТПРАВЛЕННЫЕ: иначе счётчик сервера и сумма счётчиков
      // соединений расходятся ровно на пропуски, и админ, читающий их рядом
      // (SRV-4), делил бы байты на снапшоты, которых не было. Хост этому
      // счётчику уже хозяин наравне с `bytesSent`.
      this.server.metrics.snapshotsSent--;
      return -1;
    }
