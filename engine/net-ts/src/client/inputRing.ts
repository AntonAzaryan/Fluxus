 ...
 *
 * Два РАЗНЫХ кадра на один тик одной эпохи — штатный случай, а не ошибка:
 * подтягивание оценки тика вниз (`resyncTick`, NTR-10) и спуск запаса разметки
 * (`lead.ts`, NTR-7) заставляют клиента пометить следующий кадр тем же тиком,
 * что и предыдущий. Сервер на пару (слот, тик) применяет ПЕРВЫЙ принятый кадр,
 * а последующие отбрасывает (NTR-7, «Повторный фрейм»), поэтому и кольцо на
 * такой тик держит первый: эхо `seq` в снапшоте вернёт именно его, и кадр,
 * которого сервер не применял, не должен заслонять тот, который применил.
 * Иначе подтверждение не находилось бы в кольце вовсе — ни для метрики
 * отклика, ни для контроллера запаса, и тот судил бы о спуске сроком эха, а не
 * разностью тиков (десятки тиков повтора вместо единиц).
 */
...
export class InputRing {
  ...
  push(frame: InputFrame, sentAtMs: number, epoch: number): void {
    const index = frame.tick % this.slots.length;
    const held = this.slots[index];
    // Первый выигрывает — то же правило, что у сервера (NTR-7): второй кадр на
    // тот же тик той же эпохи сервер отбросит, и в кольце ему нечего помнить.
    if (held?.frame.tick === frame.tick && held.epoch === epoch) return;
    this.slots[index] = { frame, sentAtMs, epoch };
  }
