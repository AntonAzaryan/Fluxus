...
export class MatchClient {
   ...
   * предела. Сама граница — «тик снапшота плюс период рассылки плюс
   * `inputDelay`» — нормирована NTR-10 («Оценка серверного тика») как правило
   * ПОДТЯГИВАНИЯ, а не как непрерывный потолок: между снапшотами оценку ведёт
   * `advance()` собственным темпом клиента, и удерживать её там нечем — часов
   * сервера у клиента нет. Выбрана она не этой сборкой: на неё опирается
   * потолок адаптивного запаса разметки (NTR-7), и вторая реализация,
   * выбравшая другой запас, получила бы другую границу потери ввода. Кадры уезжали бы всё дальше в будущее, пока не начали бы выпадать
