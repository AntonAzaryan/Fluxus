...
export class MatchClient {
  ...
  private onWelcome(
    ...
    // Лобби — только из приветствия. Канал порядок не гарантирует (NTR-2), и
    // `Start`, отправленный в одной рассылке с `Welcome` последнему вошедшему,
    // вправе обогнать его: матч тогда уже идёт, и возвращать клиента в лобби
    // значило бы оставить его там до конца матча — ввод не уходит, снапшоты
    // применяются молча.
    if (this.clientPhase === 'greeting') this.clientPhase = 'lobby';
  }
  ...
  private measureResponse(snapshot: Snapshot, nowMs: number): void {
    ...
    const sent = seq > 0 ? this.ring.bySeq(seq) : undefined;
    // Своего кадра в этом состоянии нет: `seq` нулевой — сервер ещё ни одного
    // кадра слота не применял и повторяет нулевой (TICK-2), — либо не из
    // кольца: вытеснен, или не наш вовсе — возвращение в матч поднимает НОВЫЙ
    // клиент с нумерацией с начала (NTR-17), а в мире до первого своего
    // применённого кадра стоит `seq` прежней сессии. Отклик из такого состояния
    // не выводится. Контроллеру запаса оно всё же уезжает — ТИКОМ снапшота, без
    // подтверждённого кадра (`lead.ts`, `noEcho`): сервер прошёл этот тик, и ни
    // один из наших кадров к нему не применён. На старте длинного канала это
    // единственное наблюдение, которое приходит раньше срока эха.
    if (sent === undefined) {
      this.leadControl?.noEcho(snapshot.tick);
      return;
    }
