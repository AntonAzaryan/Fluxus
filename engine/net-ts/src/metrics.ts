...
export interface ClientMetrics {
  ...
  /**
   * Пачки тиков потока событий (NTR-15), применённые курсором и отданные
   * потребителю: сколько фактов доехало и было проиграно.
   */
  eventBatchesDelivered: number;
  /**
   * Пачки, отброшенные курсором как уже применённые, — пара `(эпоха, тик)` не
   * больше курсора (NTR-16). Это НЕ дефект канала, а цена избыточности:
   * каждое сообщение повторяет `eventRepeat` предыдущих рассылок (NTR-15), и
   * на канале без потерь повторов ровно столько, сколько послано. Сюда же
   * попадают пачки стёртой перемоткой эпохи, доехавшие после смены эпохи:
   * принять их нельзя, а доехали они впустую — как и всякий повтор.
   */
  eventBatchesDropped: number;
  /**
   * Разрывы диапазона (NTR-15): сообщение, диапазон которого начинается позже,
   * чем кончился последний применённый, в той же эпохе. Отдельный счётчик, а
   * не слагаемое сводной «потери»: разрыв означает потерянные безвозвратно
   * факты, тогда как отброшенный повтор не означает ничего, кроме работающей
   * избыточности. Рост эпохи разрывом не считается — перемотка наблюдаема с
   * обеих сторон и дефектом канала не является.
   */
  eventRangeGaps: number;
...
export function createClientMetrics(): ClientMetrics {
  return {
    ...
    inputsSent: 0,
    inputsStranded: 0,
    eventBatchesDelivered: 0,
    eventBatchesDropped: 0,
    eventRangeGaps: 0,
    bytesReceived: 0,
