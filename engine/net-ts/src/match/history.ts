 ...
 *
 * Ничью РАВНЫХ тиков кольцо разрешает само и в нужную сторону: «Побеждает
 * записанный ПОЗЖЕ» (`sim/history.ts`), то есть живая ветвь, — и записывать
 * здесь второе правило для того же случая было бы вторым источником одной
 * величины. Нужна обрезка ради другого: снапшоты стёртой ветви остаются в
 * кольце с тиками ВЫШЕ точки восстановления, и живой ветви от этого хуже
 * втройне. Они занимают ёмкость, вытесняя самые старые её снапшоты, — то есть
 * молча поднимают нижнюю границу доступной истории, которой ограничена
 * перемотка (REW-1). Они называют собой `newestTick`. И они остаются дном
 * кольца — снапшотом минимального тика, которым `nearest()` отвечает на запрос
 * глубже буфера: после глубокого отката живых снапшотов ниже них может не
 * оказаться вовсе, и ответом станет состояние ветви, в которой матч больше не
 * находится (REW-13), да ещё с тиком выше запрошенного.
 *
