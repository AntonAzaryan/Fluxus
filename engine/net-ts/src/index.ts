 ...
 * ядро о сети не знает (DI-6), и модуль делает это проверяемым сборкой, а не
 * ревью.
 *
...
export {
  ...
  PAUSE_DENY_REASONS,
  PAUSE_STATES,
  // По тому же основанию — закрытые наборы исходов отказа и конца матча
  // (NTR-4): их проверяет разбор, и сборке, показывающей игроку эти исходы,
  // перечень нужен значением, а не пересказом.
  END_REASONS,
  REJECT_REASONS,
