...
// (`matchConfigOf`), включая зависимости сборки мира (NTR-14) и историю
// перемотки (NET-11). Своё здесь — решения запуска, а не свойства матча:
// наблюдатель, трейс и то, куда уходит диагностика перемотки.
const server = new MatchServer({
  ...matchConfigOf(match, pack),
  allowObserver: flag('observer'),
  // Диагностика запроса перемотки — в СОБСТВЕННЫЙ отчёт запускалки, то есть в
  // stdout: умолчание канала пишет в stderr, а туда же по умолчанию идёт трейс,
  // и артефакт со строками постороннего происхождения разбирается человеком, а
  // не программой (CLI-11).
  rewindWarn: (message) => process.stdout.write(`${message}\n`),
