...
describe('эпоха рядом с кадром (NTR-10)', () => {
  ...
  it('перемотка не выметает кольцо: стёртая эпоха остаётся счётной диагностикой', () => {
    const ring = new InputRing(8);
    // Три кадра ушли до первой перемотки, два — после возобновления, ещё два —
    // после второй перемотки.
    for (let tick = 1; tick <= 3; tick++) ring.push(frame(tick), tick * 16, 0);
    for (let tick = 4; tick <= 5; tick++) ring.push(frame(tick), tick * 16, 1);
    for (let tick = 6; tick <= 7; tick++) ring.push(frame(tick), tick * 16, 2);

    // Ровно столько ввода унесла первая перемотка — и это единственное, ради
    // чего кадры стёртой эпохи в кольце остаются: переотправке они не подлежат.
    // Кадры живых эпох в счёт не идут, иначе величина отвечала бы на вопрос
    // «сколько всего отправлено», а не «сколько унесла перемотка».
    expect(ring.strandedBefore(1)).toBe(3);
    // Вторая перемотка уносит и то, что успело уйти в эпохе 1.
    expect(ring.strandedBefore(2)).toBe(5);
    expect(ring.at(2)?.epoch).toBe(0);
    expect(ring.at(7)?.epoch).toBe(2);
  });
});
