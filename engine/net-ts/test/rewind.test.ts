 ...
 * серверу, темп двигает тест. Кадр адресуется тику `tick + inputDelay` — нижней
 * границе адаптивного запаса разметки клиента (NTR-7); сама адаптация к
 * предмету этой проверки отношения не имеет, а на канале без плеча запас с этой
 * границы и не уходит.
 */
