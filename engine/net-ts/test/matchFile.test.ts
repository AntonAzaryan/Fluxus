...
function document(overrides: Partial<MatchDocument> = {}): MatchDocument {
  return {
    ...
    inputWindow: 15,
    eventRepeat: 2,
    // Предикат видимости события (NET-13) — поле ДОКУМЕНТА: имя из закрытого
    // набора, а не функция сборки запуска. Названное здесь имя совпадает с
    // нормированным NET-13, и предмет проверки — что оно доезжает до конфига,
    // а не то, как отбирает предикат.
    eventVisibility: 'any-referenced',
    silenceSeconds: 30,
    rewind: REWIND,
    pause: PAUSE,
    scenes: { duel: scene() },
...
describe('раскладка зависимостей сборки для клиента (NTR-14)', () => {
  ...
  const SERVER_ONLY_FIELDS: Readonly<Record<string, string>> = {
    ...
    eventVisibility:
      'отбор событий по видимости делает сервер, вызывая фильтр ядра (NET-13, NTR-9)',
  };
