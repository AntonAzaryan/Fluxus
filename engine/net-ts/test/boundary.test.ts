 ...
 * (DI-6), и обратный импорт обязан быть невозможен физически.
 *
...
describe('поверхность ядра не расширялась', () => {
   ...
   * в `export const` — идиому, которой само `index.ts` пользуется ровно для
   * одного имени: `worldInitSpawn`, мутатора ECS-уровня из исключения 1 TICK-3.
   * Второе экспортируемое исключение (применение снапшота, см. шапку выше) этой
   * формы не имеет и разбору не предмет, поэтому идиома разрешена ровно
   * `worldInitSpawn`.
   *
