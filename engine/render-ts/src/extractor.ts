...
/**
 * Колонок плоской формы НА СУЩНОСТЬ — множитель объёма, который канал доставки
 * копирует за тик (`extractChannelValues`, PERF-2): `id`, `kind`, `x`, `y`,
 * `level`, `flags`, `facingYaw`, `aimYaw`, `motion`, `motionPhase`,
 ...
 * заводится, — и подорожание канала видно в диффе эталона стоимости.
 *
 * Расхождение с раскладкой кодека стережёт тест на стороне оболочки, где эта
 * раскладка и живёт («сущность стоит каналу ровно объявленных колонок»,
 * `client-ts/test/codec.test.ts`): молча разойтись двум числам нельзя.
 */
