...
export class LightingSubsystem implements RenderSubsystem, ShadowCasterSink {
  ...
  setCaster(root: THREE.Object3D, tier: ShadowCasterTier): void {
    ...
    if (!moved && tier === 'dynamic') {
      ...
      //
      // Фаза `none` — единственная, где адресности мало: она означает «фазы
      // ещё не было» (значение по умолчанию и сброс на смене конфигурации), и
      // будущий `applyPhase` обязан застать этот корень в реестре — что он и
      // делает, потому что реестр правится до него.
      mine.add(root);
      if (this.phase !== 'none') {
