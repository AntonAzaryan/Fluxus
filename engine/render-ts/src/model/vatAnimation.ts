...
export class VatAnimationBackend implements AnimationBackend {
  ...
  /**
   * Кадр: `dt` — часы презентации СО ЗНАКОМ (REND-25), `pace` — персональная
   * шкала времени сущности (REND-38, умолчание — единица). Отрицательный шаг
   * отматывает клип назад, нулевой замораживает его на текущей фазе; шкала
   * меняет только СКОРОСТЬ подхода фазы к границе клипа, а не саму границу —
   * развязки one-shot'а (конец вперёд, начало назад) остаются прежними, как и
   * паритет ярусов (REND-20).
   */
  update(dt: number, pace = 1): void {
    if (this.fadeLeft > 0) {
      // Кроссфейд дренируется по МОДУЛЮ и БЕЗ шкалы: переход между клипами —
      // не часть прошлого мира, а собственный переход картинки. Отматывать его
      // назад значило бы подвесить пару поз на всё время перемотки, а
      // масштабировать персональной шкалой — растянуть его впятеро у
      // замедленной сущности (REND-25, REND-38).
    ...
    // Фаза клипа — то, что показывает ход мира у сущности, и она единственная
    // здесь идёт персональным темпом (REND-38).
    if (!this.clamped) this.advancePhase(clip, dt * pace);
    this.recompute();
  }
