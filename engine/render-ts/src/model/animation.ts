...
export interface AnimationBackend {
   ...
   *
   * `pace` — персональная шкала времени сущности (`time-system` TIME-2,
   * REND-38), множитель ФАЗЫ клипов; неотрицателен, знака не несёт (его задаёт
   * только режим мира) и по умолчанию равен единице — темпу сущности без
   * шкалы (TIME-3) и всякого потребителя, который о шкале не знает. Переходы,
   * принадлежащие картинке, а не миру (кроссфейд), ему НЕ подчиняются: иначе
   * замедленная сущность вечно втекала бы в новый клип (REND-38, REND-25).
   */
  update(dt: number, pace?: number): void;
  /**
...
export class MixerAnimationBackend implements AnimationBackend {
  ...
  private current = -1;
  /**
   * Действия, заведённые этим бэкендом: по ним разносится ход клипов —
   * направление (REND-25) и персональная шкала сущности (REND-38). Набор
   * ограничен числом клипов модели и растёт только на смене клипа, а не в
   * кадре.
   */
  private readonly actions = new Set<THREE.AnimationAction>();
  /**
   * Ход клипов действий: знак — направление часов (+1 вперёд, −1 назад;
   * ноль часов — не направление, а стоп), модуль — персональная шкала времени
   * сущности (REND-38). Начальное значение — обычный темп вперёд.
   */
  private clipScale = 1;
  ...
  /**
   * Кадр микшера. Ход клипа берёт ДЕЙСТВИЕ (`timeScale`) — и направление
   * (REND-25), и персональную шкалу сущности (REND-38), — а сам микшер идёт
   ...
   * весе до конца перемотки. По той же причине шкала конвертов не касается:
   * переход принадлежит картинке, а не миру, и замедленная сущность обязана
   * доигрывать его за штатную длительность. Пауза (`dt === 0`) останавливает и
   * то и другое.
   *
   * Перезапись — по изменению ПРОИЗВЕДЕНИЯ: шкала под твином ауры меняется
   * каждый кадр, но цикл идёт по действиям клипов ЭТОЙ модели (их единицы), а
   * не по сущностям, и аллокаций не делает.
   */
  update(dt: number, pace = 1): void {
    const scale = (dt < 0 ? -1 : 1) * pace;
    if (scale !== this.clipScale) {
      this.clipScale = scale;
      for (const action of this.actions) action.timeScale = scale;
    }
    this.mixer.update(Math.abs(dt));
  }

  /** Действие клипа под учётом бэкенда: ход у него — общий текущий (REND-25, REND-38). */
  private claim(index: number): THREE.AnimationAction {
    const action = this.mixer.clipAction(this.clips[index]!);
    action.timeScale = this.clipScale;
...
export class AnimationController {
   ...
   * `dt` — часы презентации со знаком (REND-25), `pace` — персональная шкала
   * времени сущности (REND-38, умолчание — единица); ни направления, ни
   * масштаба машина не знает — они целиком у носителя воспроизведения.
   */
  update(dt: number, pace = 1): void {
    this.backend.update(dt, pace);
  }
