...
export interface RenderCostCounters {
  ...
  waterQuads: number;
  /**
   * Живые кольца ряби кадра (REND-36) — сумма по телам: столько их уехало в
   * униформы и столько же считает каждый фрагмент воды своего тела. Ось
   * стоимости фрагмента, и ровно её ограничивает потолок `water.rippleSources`
   * (QUAL-1). Кольца, а не сущности: идущий по воде юнит роняет их следом и
   * держит несколько одновременно, а ушедший из доставки (туман NET-12) ещё
   * какое-то время платит уже рождёнными. Ноль на производительном пресете
   * означает выключенную рябь, а не отсутствие воды.
   */
