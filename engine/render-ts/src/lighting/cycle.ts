 ...
 * границу слота, поэтому возврата к первой фазе скачком не бывает. Переход
 * длиннее слота укорачивается до него: недоигранный кроссфейд и был бы тем самым
 * скачком.
 *
 ...
 *
 * Ход считается по `realDt` (REND-25): длина суток — свойство картинки, и
 * замедление времени симуляции её не растягивает. Состояния симуляции цикл не
 * читает вовсе — инвариант PRES-4 держится тем, что читать здесь нечего.
 *
 * ## Аллокации
 *
...
function prepare(config: LightingCycleConfig): readonly LightingCyclePhase[] {
  ...
  return phases.map((phase, index) => {
    ...
    return {
      ...
      // Переход длиннее слота доигран не будет — укорачиваем до слота.
...
export class LightingCycle {
   ...
   *
   * Тон дописывается тем же вызовом и назван по ФАЗЕ, с которой идёт кроссфейд:
   * точный тон кадра стоит на живых источниках, а `getHexString()` с них
  ...
  fillDebug(out: DebugLightingState): void {
    ...
    out.ambientColor = phase.ambientHex;
    out.directionalColor = phase.directionalHex;
  }
