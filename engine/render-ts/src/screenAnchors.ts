...
export interface AnchorRequest {
  ...
  /**
   * Высота якоря над позой инстанса, мировые единицы, — ЯВНАЯ величина
   * потребителя. Не задана — авторская высота записи манифеста
   * (`anchorHeight`, ASSET-6), а без неё верх границ инстанса (REND-41):
   * «над макушкой» по умолчанию.
   *
   * Три источника упорядочены так, а не иначе, потому что решают разное:
   * габариты — механизм («где кончается нарисованное»), запись манифеста —
   * авторское решение о конкретной модели («у этой полоска висит выше плаща»),
   * а число потребителя — политика конкретного элемента («значок цели чуть
   * выше полоски»). Ближний к элементу источник и побеждает.
   */
  readonly height?: number;
}
...
export class ScreenAnchors {
  ...
  private write(
    ...
    // Высота якоря: величина потребителя → авторская высота записи (ASSET-6)
    // → верх границ инстанса. Границы несут масштаб записи манифеста,
    // `pose.scale` — масштаб набора (REND-11), и на авторскую высоту он
    // действует тем же множителем: увеличенный вдвое юнит носит полоску вдвое
    // выше, иначе она уехала бы ему в грудь.
    const height =
      request.height
      ?? (view.anchorHeight ?? (view.bounds === null ? 0 : view.bounds.maxZ)) * instance.scale;
    const wx = instance.x;
    const wy = instance.y;
