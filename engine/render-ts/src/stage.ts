...
export class PresentationStage {
   ...
   * геометрия приезжает инициализацией (REND-8), а не продюсером.
   */
  ...
  /**
   * Продюсер владеет состоянием (или им не владеет никто). Кадр рисует
   * активный: неактивный продюсер, чей цикл кадров ещё крутится, не должен
   * гнать подсистемам второй `updateFrame` на том же кадре.
   */
  isActive(producer: PresentationProducer): boolean {
    return this.producer === null || this.producer === producer;
  }
   ...
   * остаётся ничьей. Возврат к потоку тиков или к документу — обычный `publish`.
   */
