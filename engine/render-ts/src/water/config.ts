 ...
 * загрузив ни одного текстурного ассета, и путь демо именно этот.
 *
 ...
 *
 * - `sources: 8` — половина верхнего предела: толпа в воде на дуэльной арене
 *   столько и даёт, а потолок пресета режет это число вниз (QUAL-1);
 * - `wavelength: 1.2` и `speed: 1.6` — кольцо шире юнита и обгоняет его шаг:
 *   рябь читается следом, а не облаком вокруг;
 * - `decaySeconds: 1.6` — след живёт около двух шагов и не копится в кашу;
...
export const WATER_SHININESS = 96;

/** Сила блика направленного света сцены (REND-29) на поверхности воды. */
export const WATER_SPECULAR = 1.2;

/**
 * Непрозрачность на глубине полного цвета. У самого берега вода почти
 * прозрачна — дно должно читаться, иначе мелководье выглядит крашеным стеклом.
 */
