 ...
 * (или в кольце в одну клетку вокруг), чья скорость выше порога и которая не в
 * воздухе: прыжок (LOC-3) и полётная дуга (REND-12) колец не поднимают — рябь
 * рождает касание поверхности, а не пролёт над ней (REND-36). Новых входов
...
export class WaterRippleField {
  ...
  private collect(view: TickView, alpha: number, options: WaterRippleOptions): void {
    ...
    for (const entity of view.entities.values()) {
      ...
      // поверхности. Возраст пропущенной снимет prune — приземление начнёт
      // кольцо заново, а не докрутит накопленное до полёта.
