...
export function cornerLevels(
   ...
   *
   * Притяжение ВНИЗ к соседу без рампы после нормирования стороны флага
   * (TERR-5) — защитная ветвь: на карте, прошедшей загрузку, нижний сосед
   * рампы сам помечен рампой, иначе ассет отвергается. Смыкания на такой паре
   * REND-9 поэтому и не обещает — ветка держится как защита от сетки, которой
   * в рантайме не бывает, а не как исполняемое правило.
   */
