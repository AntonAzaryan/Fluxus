...
// ------------------------------ нагрузка оборота устоявшегося мира (PERF-9)

/**
 * Сколько тиков живёт сущность оборота. Она же — численность устоявшегося мира:
 * спавн идёт по одной на тик, значит после прогрева живых ровно столько, сколько
 * тиков жизни, и слоты возвращаются с той же частотой, с какой берутся.
 */
export const CHURN_LIFETIME = 12;

/**
 * Сцена постоянного ОБОРОТА (PERF-9): каждый тик рождается сущность с тегом и
 * умирает самая старая. Механизм проверки, а не контент движка: нагрузка живёт
 * в тесте, потому что проверяется ею не игра, а возврат — слотов сущностей
 * (ID-6), записей тегов, команд буфера.
 *
 * Системы описаны JSON'ом намеренно (SYS-1): оборот, устроенный нативной
 * системой, проверял бы путь, которым геймплей не идёт, — а команды спавна и
 * гибели в игре заказывает именно evaluator.
 */
export function churnScene(): SceneDef {
  const life = (entity: string): Record<string, unknown> => ({
    getComponent: [{ var: entity }, 'Life', 'ticks'],
  });
  return {
    components: [{ name: 'Life', fields: { ticks: 'i32' } }],
    prefabs: [{ name: 'Mote', tags: ['Mote'], components: { Life: { ticks: CHURN_LIFETIME } } }],
    systems: [
      // Рождение — одно на тик, вне запроса: действие без `query` исполняется
      // ровно раз (SYS-1).
      { name: 'Spawner', order: 10, do: [{ spawnEntity: { prefab: 'Mote' } }] },
      {
        name: 'Aging',
        order: 20,
        query: { all: ['Life'] },
    ...
    // Ёмкости с запасом на два поколения: исчерпание её было бы отказом ядра
    // (ID-1), а не свойством нагрузки.
    capacity: 4 * CHURN_LIFETIME,
  };
}
