...
describe('фазз по списку seed’ов (CLI-9)', () => {
  it.each(SEEDS)('seed %d: прогон воспроизводим и реплеится побитово (NTR-8)', async (seed) => {
    ...
    // Срок — свой, а не умолчание в 5000 мс: на один seed приходятся два полных
    // матча и реплей всего канонического лога, и на медленной машине это
    // занимает дольше. Медленная машина обязана делать фазз медленным, а не
    // красным: предмет проверки — побитовое совпадение, а не длительность.
  }, 60_000);
  ...
  it('разные seed’ы дают разные потоки ввода, а не пять копий одного', async () => {
    ...
    // Тот же срок и по той же причине: здесь прогоняются все пять seed'ов.
  }, 60_000);
});
