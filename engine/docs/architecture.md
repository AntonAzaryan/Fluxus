...
| `netcode` | Server-auth, предсказание, per-client фильтрация, единая версия | NET-1..16 |
...
13. **Механизм сверки версии игры** (netcode/клиент). Правило зафиксировано нормой: все участники матча исполняют одну версию — сборку вместе с контент-паком, — а расхождение отклоняет вход с исходом «версия устарела» (NET-16). Механизма нет: транспорта в репозитории не существует, а апдейт-гейт на клиенте — вне детерминированного ядра (DI-3). Открыто, чем считается сама версия (хеш контент-пака, номер сборки, их пара) и где проходит проверка. На это правило прямо опирается DET-1, исключая конфиг сцены из хеша `worldInit`: сверять то, что уже гарантировано версией, незачем. — *netcode.*
