...
| 10 | CLI + golden-file test suite | ✅ `src/scenario.ts`, `bin/sim.mjs`, `engine/tests/golden/`: прогон сценария из одного JSON, побитовая сверка (`viewpoint = ALL`). Инпуты и RNG эталонами не покрыты — в DSL нет доступа ни к тем, ни к другим |
