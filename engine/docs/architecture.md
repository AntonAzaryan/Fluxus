...
| 22 | (Будущее) Pathfinding для крипов/NPC | 🟡 Шов готов: `NavigationApi` и `navigation?` в `SystemContext` (`pathfinding` NAV-1..6, DI-4). Алгоритм, навигационные данные и связность сетки — отдельным этапом |
| 23 | (Будущее) Предсказание своей сущности и reconciliation | NET-2/4/5 слоем над клиентом MVP; включается по замеру «нажал→увидел», а не заранее |

Этапы с 21-го появились после составления roadmap и потому идут за «будущим» 20-м: ни транспорт, ни навигация не были условием ни одного этапа ядра.
