...
- [x] 3.1 Каркас пакета `engine/integration-ts` (workspace-участник, зависимости на все четыре пакета, vitest + typecheck в общем прогоне)
- [x] 3.2 Фикстуры вертикали через публичные API: loopback-матч + `RenderHost` без импорта чужих `test/`
- [x] 3.3 Вертикальный смоук: ввод игрока 1 доезжает до позиции меша у клиента игрока 2 в соответствии с интерполированным снапшотом
- [x] 3.4 Фазз-прогон: генерация потоков ввода от фиксированного seed через публичный RNG ядра; проверка воспроизводимости и реплей-парности (NTR-8) в бюджете тиков

## 4. Golden матчей (CLI-10)

- [x] 4.1 Рекордер в `integration-ts`: разыгрывает детерминированный матч и seeded-фазз, снимает `toScenario()`, пишет пары `match-*.scenario.json` / `match-*.golden.json` в `engine/tests/golden/`
- [x] 4.2 Записать первые фикстуры (минимум: матч с движением двух игроков, фазз-матч); убедиться, что адаптер `golden.test.ts` подхватил их и `npm run golden` перегенерирует
- [x] 4.3 Приёмка сценария CLI-10: искусственная правка пейсинга в `net-ts` красит golden матча при зелёных эталонах ядра

## 5. Финализация

- [x] 5.1 `openspec validate --specs --strict` и полный прогон `npm test --workspaces` зелёные; сверить дельту с реализацией перед архивированием change
