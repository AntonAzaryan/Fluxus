...
export class NetworkShell {
  ...
  private unmapped = 0;
  /**
   * Последнее ОТПРАВЛЕННОЕ главному потоку состояние паузы (NTR-20). Сравнение
   * с текущим держит каденс конверта: состояние паузы меняется единицы раз за
   * матч, а шаг оболочки идёт в темпе тиков — слать одно и то же шестьдесят раз
   * в секунду значило бы завести второй поток доставки рядом с кадрами.
   */
  private pause: DeliveredPause | undefined;
  ...
  reattach(client: MatchClient, transport: Transport): void {
    ...
    this.pending = [];
    // Состояние паузы за разрыв могло смениться (NTR-20): пауза, стоявшая до
    // него, могла быть снята без нас. Отправленное забывается, чтобы то, что
    // объявит сервер вернувшемуся (NTR-17, D8), доехало до главного потока даже
    // если совпадёт с прежним; до его прихода главный поток держит последнее
    // известное — придумывать за сервер здесь нечего.
  ...
  step(): void {
    ...
    // Состояние паузы — до применения доставок и независимо от них (NTR-20):
    // в заморозке доставок нет вовсе, и привяжи мы конверт к ним, пауза
    // доехала бы только с возобновлением.
    this.deliverPause();
  ...
  private applyLatest(): void {
    ...
    // Режим ставится здесь отдельным шагом: `restoreSnapshot` машину состояний
    // не восстанавливает (`snapshot-rewind` REW-2), и это не восстановление, а
    // ФАКТ, приехавший со снапшотом, — режим авторитетного сервера (NET-1,
    // NET-18, NTR-10). Присваивание идёт тем же исключением 4 TICK-3, каким
    // наполняется весь этот мир: тика здесь нет вовсе (SHELL-8).
  ...
  /**
   * Состояние паузы главному потоку (SHELL-9, NTR-20) — только на СМЕНЕ:
   * конверт несёт то, что сервер сообщил, а сообщает он единицы раз за матч.
   *
   * Отказ (`deniedSeq`) участвует в сравнении наравне с состоянием: два
   * одинаковых отказа подряд оставляют состояние прежним, и без номера второй
   * до главного потока не доехал бы вовсе — игрок нажал бы паузу дважды и
   * второй раз не получил бы ответа (SHELL-9, HUD-9).
   */
  private deliverPause(): void {
    const pause = this.client.pause;
    // Состояния паузы не доставляли — сообщать нечего. Молчание не
    // превращается в «матч идёт» ни здесь, ни у потребителя: доставленное
    // «идёт» есть утверждение сервера, а его отсутствие — отсутствие
    // утверждения (SHELL-9, HUD-9).
    if (pause === undefined) return;
    const sent = this.pause;
    if (
      sent?.state === pause.state &&
      sent.slot === pause.slot &&
      sent.countdownMs === pause.countdownMs &&
      sent.deniedSeq === pause.deniedSeq
    ) {
      return;
    }
    this.pause = pause;
    this.config.port.post({ t: 'pause', ...pause });
  }
  ...
  /**
   * Запрос перехода машины состояний уходит на сервер (SHELL-6, `netcode`
   * NET-11, REW-6), и путей у него два — по тому, чем этот переход является.
   *
   * `pause`/`resume` — ПАУЗА МАТЧА (NTR-20): свой тип закрытого набора
   * (`PauseRequest`), потому что кадры замороженного мира сервер отбрасывает
   * целиком, и «снять паузу» битом ввода не доехало бы ровно в том состоянии,
   * ради которого посылается. Именованный отказ политики возвращается тем же
   * `Pause` и доезжает до HUD конвертом состояния паузы (SHELL-9, HUD-9).
   *
   * `beginRewind`/`seekTo` — ульта отката: уезжает ВВОДОМ, тем же кольцом
   * инпутов и тем же сообщением `Input`, что обычный ввод (NET-7): для сервера
   * включение ульты неотличимо от прочих нажатий. Неотображённое в кнопку
   * действие отбрасывается со счётчиком и локально не исполняется — исполнить
   * его тут было бы ровно то, что SHELL-6 запрещает.
   *
   * Тик из `seekTo` теряется намеренно: точку остановки ведёт инициатор ульты на
   * сервере (`snapshot-rewind` REW-7), а канала для «перемотай на тик N» нет и
   * быть не должно.
   */
  private onControl(message: ControlMessage): void {
    if (message.action === 'pause' || message.action === 'resume') {
      this.client.requestPause(message.action);
      return;
    }
