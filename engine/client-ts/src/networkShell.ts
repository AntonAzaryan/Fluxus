...
export class NetworkShell {
  ...
  /**
   * Состояние паузы главному потоку (NTR-20) — только на СМЕНЕ: конверт несёт
   * то, что сервер сообщил, а сообщает он единицы раз за матч.
   *
   ...
   * второй раз не получил бы ответа (HUD-9).
   */
   ...
   * `Pause` и доезжает до HUD конвертом состояния паузы (HUD-9).
   *
