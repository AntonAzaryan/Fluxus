...
import type { DeliveredPause } from '@fluxus/net';
...
/**
 * Состояние паузы матча, доставленное главному потоку (SHELL-9,
 * `netcode-transport` NTR-20) — третий вид сообщения канала наравне с handshake
 * (SHELL-5) и конвертом тика (SHELL-3, SHELL-4).
 *
 * Своим конвертом, а не полем кадра тика, и это не выбор раскладки: в заморозке
 * доставок состояния нет вовсе — сервер живых тиков не исполняет и снапшотов не
 * рассылает, — поэтому пауза, приехавшая полем кадра, доехала бы только с
 * возобновлением. То есть ровно тогда, когда она уже не нужна, а HUD всё это
 * время выводил бы её из молчания канала — из того самого признака, который
 * HUD-9 запрещает: молчание канала есть картина сбоя сети, а не паузы.
 *
 * Конверт conflatable по построению: состояние паузы одно на матч, и доехавшее
 * позже описывает его точнее. Не конфлатируется только отказ — `deniedSeq`
 * растёт на каждом, чтобы второй отказ с той же причиной отличался от первого.
 *
 * Локальный режим оболочки (`WorkerShell`) этого конверта не шлёт: пауза матча
 * — механизм сервера (NTR-20), а локальная пауза есть переход WSM, который
 * главный поток и так видит в `mode` доставленного кадра (SHELL-7, SHELL-8).
 */
export interface PauseEnvelope extends DeliveredPause {
  readonly t: 'pause';
}

export type WorkerToMain = HelloMessage | TickEnvelope | PauseEnvelope;
...
export interface InputMessage {
  ...
  /**
   * Точка прицела (TICK-2) — уже fixed-пара, как и `move`: конверсию делает
   * отправитель, канал возит канонические величины. Формат сообщения от неё не
   * меняется — это ещё одно поле ввода рядом с движением и направлением, а не
   * новый вид сообщения; `undefined` — источник точкой не владел.
   *
   * Биты подтверждения и отмены шага прицеливания собственных полей не
   * получают вовсе: они обычные биты `buttons` (INP-4).
   */
 ...
 * у себя через core-API (WSM-5), в сетевом отправляет его авторитетному серверу
 * и у себя MUST NOT исполнять.
 *
 * Форма сообщения одна на оба режима (SHELL-6), а вот путь запроса в сетевом
 * режиме зависит от того, чем переход является, и выбирает его воркер-сторона:
 * перемотка уезжает ВВОДОМ (`netcode` NET-11, `snapshot-rewind` REW-6, REW-13),
 * пауза матча — сообщением `PauseRequest` (`netcode-transport` NTR-20), потому
 * что кадры ввода замороженного мира сервер отбрасывает целиком. Главный поток
 * этой разницы не знает и знать не должен.
 *
 * «Запрошено» и «произошло» разделены кругом до сервера, и главный поток узнаёт
 * о результате из доставленного состояния (SHELL-7) — а о состоянии паузы из
 * своего конверта (SHELL-9).
 */
