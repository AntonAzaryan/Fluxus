 ...
 * кроме наблюдений: разрыв непрерывности доставки (SHELL-7) и смена режима мира
 * (WSM-1) — сигнал «то, что помнил, больше не про этот мир». По нему память
 * сбрасывается целиком; без сброса очередь наблюдений и правило «картинка назад
 * не ходит» удерживали бы мозг на СТЁРТОМ будущем на всю глубину отката.
 */
...
export class Perception {
  ...
  observe(step: ClientStep): void {
    ...
    if (view.discontinuity || view.mode !== this.deliveredMode) {
      this.broken = true;
      this.forgetBranch();
    }
