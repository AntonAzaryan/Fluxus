...
describe('бот на паузе матча (NTR-20, D9)', () => {
  it('мозг не зацикливается и не копит очередь на длинной паузе', async () => {
    ...
    bots.dispose();
    // Срок — свой, а не умолчание в 5000 мс: тест прогоняет три сотни кругов
    // лупбэка, и под полным гейтом, где пакеты идут разом, он занимает вдвое
    // больше. Медленная машина обязана делать его медленным, а не красным, —
    // предмет проверки от длительности не зависит вовсе.
  }, 30_000);
