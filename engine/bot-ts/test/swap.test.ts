/**
 * Заменимость мозга (BOT-2, design D6): одна сборка бот-хоста, две реализации
 * за контрактом — различие исчерпывается фабрикой.
 *
 * Это и есть проверяемая форма утверждения «замена classic → ML — замена
 * сборки»: если бы хостинг знал про реализацию, тест не собрался бы дважды
...
import { connectBot, connectHuman, duelConfig, harness, settle, stepMatch, testProfile } from './fixtures.js';
...
describe('замена реализации мозга — замена сборки (BOT-2)', () => {
  it('скриптовый и классический входят в матч одинаково', async () => {
    const scripted = await playWith(walkToCenter);
    const classic = await playWith(classicBrain());

    for (const played of [scripted, classic]) {
    ...
    expect(JSON.stringify(classic.frames)).not.toBe(JSON.stringify(scripted.frames));
  });

  it('скриптовый мозг воспроизводим: два прогона — один поток ввода (D6)', async () => {
