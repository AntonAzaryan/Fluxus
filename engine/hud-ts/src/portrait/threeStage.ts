 ...
 * (REND-3).
 *
 * Что здесь проверяется тестами и что нет. Настоящий WebGL-контекст нужен
 * стенду ровно в двух местах: `attach`, где заводится `WebGLRenderer`, и
 * висящий на нём цикл кадра (`frame` и обращения к `requestAnimationFrame`/
 * `cancelAnimationFrame`) — их в Node нет, и `ensureLoop` это прямо проверяет.
 * Только они и остаются на typecheck и глаза.
 *
 * Всё остальное живёт при `renderer === null` и покрыто без браузера
 * (`test/portraitStage.test.ts`): сборка модели, скин через ОБЩИЙ asset-сервис,
 * клип по таблице манифеста, числа записи (масштаб, скрытые части) и — главное
 * — освобождение СВОИХ ресурсов, утечка которых не видна ни typecheck'ом, ни
 * глазами, потому что картинка от неё не меняется.
 *
 * Логика самого виджета сюда не заходит вовсе: она вынесена за шов
 * `PortraitStage` и проверяется со стендом-заглушкой (`test/portrait.test.ts`).
 */
