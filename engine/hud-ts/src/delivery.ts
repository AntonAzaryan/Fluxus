 ...
 * `render-ts`, продублированные сознательно: этот модуль от `@fluxus/render`
 * не зависит (пакет берёт рендер только ради реестра качества и миникарты), а
 * структурная типизация TS делает адаптер HUD совместимым с
 * `RemoteHost.register` без импорта типов рендера. Сужение
...
/**
 * Доставленное состояние паузы матча (`netcode-transport` NTR-20) глазами HUD.
 *
 * Структурное подмножество конверта оболочки (`PauseEnvelope` client-ts),
 * продублированное сознательно — по тому же основанию, что и остальные типы
 * этого модуля: типов рендера и оболочки модуль не импортирует, а смысл сужения
 * в том, что виджет видит ровно перечисленное.
 *
 * Причина отказа здесь — НЕПРОЗРАЧНЫЙ ключ, а не текст: словарь «причина →
 * фраза» принадлежит игре и приезжает параметрами композиции (HUD-4), ровно как
 * таблица маркеров миникарты. Строку, зашитую в виджет, пришлось бы переводить
 * правкой пакета.
 */
export interface HudPauseState {
  readonly state: 'running' | 'frozen' | 'resuming';
  /** Слот-инициатор; `-1` — сервер: пауза обвязки стенда или админа. */
  readonly slot: number;
  /** Объявленная длительность обратного отсчёта возобновления, мс; вне `resuming` — 0. */
  readonly countdownMs: number;
  /** Ключ причины отказа последнему запросу этого клиента; нет поля — отказа не было. */
  readonly denied?: string;
  /** Номер отказа: растёт на каждом. Два одинаковых отказа подряд иначе неразличимы. */
...
export interface HudDeliveredEvent {
  ...
  /**
   * Полезная нагрузка события: координатные поля (`x`/`y`, `dirX`/`dirY`)
   * приведены к float на входной границе рендера (REND-1,
   * `render-ts/eventData.ts`), а смысл и формат прочих полей задаёт контент —
   * сцена вправе эмитить в них что угодно, включая величины симуляции в Q16.16.
   */
...
export interface HudDeliveredState {
  ...
  /**
   * Сколько событий вытеснила граница аккумулятора оболочки С ПРОШЛОЙ ДОСТАВКИ
   * (SHELL-4) — величина этой доставки, а не счётчик за матч.
   *
   * Нужна затем, зачем вытеснение и объявлено наблюдаемым: «событий не было» и
   * «события были и не доехали» — разные картины, и вторую виджет ВПРАВЕ
   * показать разрывом (SHELL-4, HUD-5), а не притвориться, что боя не было.
   * Ноль — не «не знаем», а «ничего не вытеснено»: молчаливой потери событий
   * MUST NOT быть, и её отсутствие здесь называется числом.
   */
  readonly expiredEvents: number;
  /**
   * Доставленное состояние паузы матча (NTR-20). Поля НЕТ — состояния паузы не
   * доставляли, и оверлей паузы MUST NOT его выдумывать (HUD-9): молчание
   * канала есть картина сбоя сети, а не паузы.
   *
   * Приезжает не тиком: в заморозке доставок состояния нет вовсе, поэтому
   * оболочка объявляет паузу своим конвертом, а исполнитель кладёт объявленное
   * сюда (`HudRuntime.deliverPause`) — и виджет читает его как всякое другое
   * доставленное поле (HUD-1).
   */
  readonly pause?: HudPauseState;
...
/**
 * Декларация стоимости подписки HUD для контроллера качества (`render-quality`
 * QUAL-1, QUAL-3) — структурное подмножество `QualityDeclaration` рендера,
 * продублированное по тому же основанию, что и остальные типы этого модуля.
 *
 * Ручек у HUD нет и не предвидится (`readonly never[]`), поэтому причина
 * константности ОБЯЗАТЕЛЬНА: пустой список без названной причины контроллер
 * отвергает, и это ровно то различие, которого требует QUAL-3, — «ручек нет» и
 * «объявлено, что стоимость константна» обязаны читаться по-разному.
 */
export interface HudQualityDeclaration {
  readonly subsystem: string;
  readonly knobs: readonly never[];
...
export interface HudDeliverySubsystem {
  ...
  init(): void;
  /**
   * Стоимостная декларация (QUAL-3). Метод ОБЯЗАТЕЛЕН, хотя в контракте рендера
   * он необязателен технически: подписка HUD стоит в общем списке подсистем
   * сцены, а «метода нет» — это ни ручка, ни объявленная константность.
   */
  quality(): HudQualityDeclaration;
