...
function navigationOf(def: SimulationBuildDef, terrain: TerrainApi | undefined): NavigationApi | undefined {
  ...
  // Состав блока проверяется ПЕРВЫМ: неизвестное поле — опечатка автора
  // документа, и назвать её надо раньше, чем отсутствие террейна, которое
  // относится уже к сцене. Учёт работы в счётчиках стоимости (PERF-3) —
  // параметр вызывающей СБОРКИ, а не поле блока (SER-5): опубликованная схема
  // блока закрыта и его не называет, а документ, которому позволено отменить
  // учёт собственной работы, сделал бы счётчики стоимости величиной, которую
  // крутит автор сценария или матча. Проверка живёт здесь, а не в схеме: схема
  // опубликована, но на загрузке не применяется — строгость документа несёт
  // загрузчик (ECS-5), и молча проигнорированное поле было бы опечаткой без
  // адреса.
  if ('cost' in def.navigation) {
    throw new Error(
      'SER-5: поля "cost" у блока "navigation" не существует — учёт работы навигации ' +
        'в счётчиках стоимости (PERF-3) выбирает сборка, а не документ',
    );
  }
