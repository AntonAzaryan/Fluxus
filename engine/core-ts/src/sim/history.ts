 ...
 * rewind-ульты буфер редкий и глубокий (порядка 30 тиков между снапшотами при
 * глубине 420), противоположный профиль — частый и короткий — интерфейсом
 * допускается. Второй потребитель этого интерфейса сегодня один и он
 * авторитетный: `BranchHistory` матч-сервера
 * (`engine/net-ts/src/match/history.ts`), которой сверх кольца нужна обрезка
 * стёртой перемоткой ветви. Клиентской reconciliation профиль этого интерфейса
 * предлагаться MUST NOT (SNAP-4): снапшот здесь — всегда полный мир (SNAP-1), а
 * полного отката мира на клиенте нет (NET-10), там реплеится своя сущность по
 * кольцу собственных инпутов (NET-9). Провайдеры сидят за одним интерфейсом
 * `HistoryProvider` и в одном процессе живут независимо.
 */
