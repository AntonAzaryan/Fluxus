...
function runSystems(
  ...
  // буфером, и применить его частично уже некому. Единица атомарности — система,
  // та же, что у flush'а; поднимать буфер выше тика значило бы завести путь, на
  // котором накопленное упавшей системой доживает до следующего вызова.
