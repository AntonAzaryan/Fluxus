 ...
 * идёт событиями в `TickResult`, а исполняется внешним слоем после возврата из
 * `tick()` — в вызове наблюдателя `onTick` (OBS-2).
 *
...
export interface Simulation {
   ...
   * прямое, а политика движения NPC написана над швом и работает по обе его
   * стороны — waypoint-маршрут и прямой seek поиска пути не требуют
   * (`npc-behavior` NPC-6). Живёт здесь по той же причине, что и
...
function result(state: SimulationState, isReplay: boolean): TickResult {
  const world = state.world;
  // Живой view на dirty мира, а не копия. Окно валидности открывает возврат из
  // `tick()` и закрывает первое же меняющее состояние обращение к ядру —
  // очередной `tick()` либо восстановление при перемотке; для наблюдателя это
  // значит «до возврата из `onTick`» (OBS-3, OBS-6). Шага с именем `dispatch()`
  // норма не вводит, и окно на него не опирается.
 ...
 *
 * Режим мира НЕ восстанавливается, хотя в снапшот он входит (SNAP-1): «машина
 * состояний мира восстанавливаемым содержимым MUST NOT быть» (REW-2) —
 * режимом распоряжаются разрешённые переходы (WSM-2) и core-API (WSM-5), а не
 * содержимое снапшота. Иначе скраб выбивало бы из `Rewinding` первым же
 * `seekTo` со снапшота `Running`, а вход обратно WSM-2 разрешает только через
 * `Paused`. Вызывающему, который наполняет мир снапшотом ИЗВНЕ и режим при этом
 * получает вместе с ним (сетевая оболочка, `client-shell` SHELL-8), режим
 * присваивать собственным шагом — тем же исключением 4 TICK-3, каким он
 * наполняет мир.
 *
