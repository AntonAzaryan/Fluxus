 ...
 * выше 15 доехал бы до мира целиком: маска ложится в поле компонента ввода
 * типа `i32` (набор типов поля — `ecs-foundation` ECS-3; само объявление —
 * данные сцены, а не норма движка), и `bitTest` по нему дал бы «нажато» —
 * кнопку, которой не пережить круг через транспорт: там ту же границу держит
 * NTR-7. Молча усечённой маски не бывает ни на одной из двух границ, и эта —
 * вторая.
 */
