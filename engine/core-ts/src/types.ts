...
export interface CommandBuffer {
  ...
  /**
   * ТА ЖЕ команда записи поля, адресованная handle'ом (`data-driven-systems`
   * SYS-10): канал один и тот же (CMD-1), место в порядке применения общее со
   * строковыми (CMD-3), точечное чтение видит её наравне с ними (CMD-5).
   * Отличается только адрес: имена компонента и поля разрешены заранее, один
   * раз, и горячий цикл не платит за строковый поиск ни при заказе команды, ни
   * при её проверке, ни при применении.
   *
   * Изменяемой ссылки на колонку хранилища здесь нет и не появится: handle —
   * число, а мутация по-прежнему идёт КОМАНДОЙ и применяется на flush (TICK-3).
   */
  setFieldByHandle(entity: EntityId, handle: FieldHandle, value: number): void;
  /**
...
export const DIAGNOSTIC_CODES = [
  ...
  'RNG_BOUND_INVALID',
  // Кода для «sink бросил» здесь нет намеренно: сообщить о сломанном приёмнике
  // через него же нельзя, поэтому debug-сборка поднимает обычное исключение
  // (`debug.ts`, `deliver`), а не запись потока (DIAG-4). Объявленный, но
  // никогда не выдаваемый код обещал бы подписчику сигнал, которого не будет.
  // трейс
  'SYSTEM_BEGIN',
  'SYSTEM_END',
  'COMMAND',
...
export interface SystemContext {
  ...
  /**
   * Тот же отбор и тот же порядок (QUERY-2), что у `query`, но в буферы
   * ВЫЗЫВАЮЩЕГО: в `ids` уходят идентификаторы, в `indices` — raw-индексы их
   * слотов, возвращается число совпавших. Буферами владеет сама система и
   * переиспользует их между тиками — отсюда и смысл: результат запроса
   * перестаёт быть аллокацией размером в мир на каждый вызов. Общий
   * переиспользуемый буфер QUERY-3 запрещает по-прежнему: буфер здесь СВОЙ у
   * каждой системы, и дисциплина вложенных запросов — её собственная.
   *
   * Буфер короче числа совпавших ошибкой НЕ является: записываются первые
   * `min(число совпавших, длина буфера)` в порядке QUERY-2, а возвращается
   * ПОЛНОЕ число совпавших — по нему вызывающий растит буфер и повторяет
   * запрос. Ёмкостью считается меньшая из длин двух буферов.
   */
  readonly queryInto: (spec: QuerySpec, ids: Float64Array, indices: Int32Array) => number;
  /**
   * Чтение поля по raw-индексу слота (SYS-10): индекс — из `indices` того
   * `queryInto`, чья спецификация перечислила компонент этого поля в `all`.
   * Живость и владение доказаны отбором запроса, поэтому проверок на пути
   * чтения нет вовсе; годится индекс только внутри окна валидности результата
   * (QUERY-3), и вне его — ошибка использования, а не второй способ читать мир.
   */
  readonly getByIndex: (index: number, handle: FieldHandle) => number;
...
/**
 * История снапшотов за интерфейсом (SNAP-2): потребители с разными профилями
 * интервала и глубины получают разные реализации, а не общий компромиссный
 * буфер (SNAP-4). Оба сегодняшних потребителя — авторитетные и серверные:
 * кольцо ядра `RingHistory` под rewind-ульту и `BranchHistory` матч-сервера
 * (`engine/net-ts/src/match/history.ts`), которой сверх кольца нужна обрезка
 * стёртой перемоткой ветви. Клиентской reconciliation второй профиль
 * предлагаться MUST NOT (SNAP-4): полного отката мира на клиенте у этой сетевой
 * модели нет вовсе (NET-10).
 */
