...
/**
 * Целочисленный sqrt в Q16.16: sqrt(x/2^16) = sqrt(x*2^16)/2^16, поэтому
 * считается isqrt(a << 16). `a << 16` ограничено ~2^47 — влезает в double
 * точно, как и все промежуточные квадраты.
 *
 * Кандидат берётся из `Math.sqrt` и доводится до точного isqrt двумя
 * коррекциями. Результат от начального приближения НЕ зависит: isqrt(n) —
 * единственное целое s с s² ≤ n < (s+1)², и циклы ниже останавливаются ровно
 * на нём, каким бы ни пришёл кандидат, — детерминизм по построению (DET-2), а
 * не по свойствам libm. На деле sqrt в IEEE 754 округляется точно, и каждая
 * коррекция делает не больше шага, — прежний двоичный поиск делал 24 итерации
 * с умножением на каждой и был виден в профиле npc-stress через длину вектора
 * расхождения NPC (`lengthOf`). Rust-порт волен взять любой точный isqrt.
 */
