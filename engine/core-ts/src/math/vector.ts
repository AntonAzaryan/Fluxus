...
/**
 * Нулевой вектор нормализуется в {0,0}. Проверка явная, а не через `div`: там
 * деление на ноль — ошибка расчёта, о которой debug-сборка сообщает мягким
 * assert'ом `FIXED_DIV_BY_ZERO` (FP-4: не бросает ни в одном режиме сборки и на
 * результат не влияет — результат задаёт насыщение по знаку числителя, FP-5).
 * Здесь же это штатный случай — сущность стоит на месте, — и находка в
 * диагностике была бы шумом, а не находкой.
 */
