...
function updateArenaState(ctx: SystemContext, arena: ArenaApi, entity: EntityId): void {
  ...
  // оно у каждой сущности с `ArenaState` и на каждом тике — независимо от того,
  // дойдёт ли дело до проверки пола: «цепкость» втрое больше коллайдера не
  // становится осмысленной оттого, что сущность в полёте.
