...
export class NpcMovementSystem implements System {
   ...
   *
   * Достигнутая точка возвращает к прямому seek намеренно. Стоять на ней до
   * следующего окна нельзя по двум причинам: у документа с дистанцией контакта
   * меньше дистанции прибытия агент так и не дошёл бы до цели, а шаг, длиннее
   * остатка, давал бы перелёт и разворот на месте каждый тик.
   */
  private steer(
    ctx: SystemContext,
    handles: NpcHandles,
   ...
   * компонента; достигнутая точка держимой не считается.
   *
  ...
  private requestPath(
    ...
    // Пустой список у `found` — цель в клетке агента (NAV-5): держать нечего, и
    // прямой seek здесь и есть кратчайший путь.
