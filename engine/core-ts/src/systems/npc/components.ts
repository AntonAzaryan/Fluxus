 ...
 *
 * `sepX`/`sepY` — держимый вектор локального расхождения (NPC-6). Пересчёт его
 * идёт каденсом, между окнами применяется последний пересчитанный, — значит,
 * вектор влияет на будущие тики и ЕСТЬ состояние симуляции: перемотка обязана
 * вернуть его вместе с миром (SNAP-1), и полями компонента она делает это
 * даром. Кэшем платформы вне мира он быть не может по тому же основанию, по
 * какому сетке соседей запрещено переживать тик.
 *
 * `pathX`/`pathY`/`pathValid` — ОЧЕРЕДНАЯ точка пути и признак того, что она
 * есть (NPC-6, `pathfinding` NAV-4). Списком путь не хранится намеренно:
 * `findPath` чист и детерминирован (NAV-2), поэтому политика держит одну точку
 * и перезапрашивает путь в своём окне решений. Держится она полями компонента
 * по тому же основанию, что и вектор расхождения: точка влияет на будущие тики,
 * то есть является состоянием симуляции, и перемотка возвращает её вместе с
 * миром (SNAP-1).
 */
const NPC_AGENT_SCHEMA: ComponentSchema = {
  ...
  defaults: {
    ...
    decidedTick: -1,
    enteredTick: 0,
    // Держимой точки у свежепоставленного агента нет: до первого окна решений
    // он идёт прямым seek — тем же, что в сборке без навигации (NPC-6).
    pathValid: 0,
    pathX: 0,
    pathY: 0,
    // До ПЕРВОГО пересчёта держимый вектор — ноль (NPC-6): свежепоставленный
    // агент до своего первого окна идёт без расхождения. Внеочередного
    // пересчёта на первом тике ему не устраивается, и этим каденс свежести
    // намеренно отличается от каденса решений, где первое решение форсируется
    // признаком «ещё не решал» (NPC-4, `decidedTick < 0`).
    sepX: 0,
    sepY: 0,
