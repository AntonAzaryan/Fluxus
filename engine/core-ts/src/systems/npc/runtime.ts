 ...
 * аллокаций.
 *
 * Вытеснение БЕЗУСЛОВНО: «при переполнении ёмкости SHALL вытесняться наименьшая
 * угроза» (NPC-5) не знает оговорки о весе новичка, и она была бы наблюдаема
 * побитово — источник, начавший бить позже прочих, не попал бы в таблицу
 * никогда, сколько бы урона ни нанёс, потому что накапливать ему негде. Порог
 * смены цели и забывание, которые от такой замены защищают, — политика
 * документа поведения (NPC-2), а не молчаливое правило механизма.
 *
 * Тай-брейк равных минимумов — первый по порядку слотов: сравнение строгое, и
 * состав таблицы виден в снапшоте (CLI-6).
 */
