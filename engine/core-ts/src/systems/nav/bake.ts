...
/**
 * Байты ЗАПЕЧЁННОЙ сетки (`performance-budget` PERF-8): карты проходимости и
 * зазора. Считается по готовой сборке — на запрос пути она не влияет ничем и
 * растёт только площадью арены.
 */
export function navGridBytes(nav: NavGrid): number {
  return nav.passable.byteLength + nav.clearance.byteLength;
}
