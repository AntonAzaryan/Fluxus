 ...
 *
 * У нормировки есть и второе следствие: ребро с нулевым подъёмом
 * (`levelNeg == levelPos`) при отрицательном допуске без неё блокировалось бы в
 * обе стороны. Сегодня такие рёбра террейном не порождаются вовсе — перепад
 * нуля `passable`, и коллайдера обрыва там не возникает, — но допуск `−1`
 * оставляет их проходимыми и на случай другого источника статики.
 *
