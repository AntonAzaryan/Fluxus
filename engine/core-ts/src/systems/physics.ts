 ...
 *
 * Диапазон: расстояния, участвующие в квадратичных тестах broad-phase и луча,
 * не должны выходить за ~181 единицу — квадрат большего значения не помещается
 * в Q16.16 (PHYS-6). Предел этот местный, а не общий на всё ядро: отбор по
 * радиусу (`withinRadius`, QUERY-1) сравнивает суммы квадратов точной
 * 64-битной арифметикой на паре слов (`math/fixed.ts`) и честен на любой
 * дистанции — здесь же квадраты считаются в Q16.16 и за пределом врут.
 */
