...
/**
 * Исход для источника `cancelInput` без назначенного бита отмены — ошибка
 * загрузки (ABIL-6, ABIL-10): фронт бита есть ЕДИНСТВЕННЫЙ сигнал этого
 * источника, и объявленное определением прерывание не сработало бы никогда.
 * «Молча недействующего прерывания MUST NOT быть» — то же правило, по которому
 * отвергается источник без нужного биндинга сцены (ABIL-8).
 *
 * `declared` копит источники И определения, И его фаз: объявить исход вправе
 * любой из двух блоков, а бит назначает только определение.
 */
export function checkCancelInputBit(cancelBit: number, declared: number, path: string): void {
  if (cancelBit >= 0 || (declared & sourceMask(INTERRUPT_CANCEL_INPUT)) === 0) return;
  throw new Error(
    `${path}.cancelBit: определение объявляет исход для источника прерывания "cancelInput", ` +
      'но бита отмены не назначило (ABIL-6, ABIL-10): фронт бита — единственный сигнал этого ' +
      'источника, и прерывание не сработало бы никогда',
  );
}
