...
/**
 * Шаг прицеливания, берущий точку прицела ввода (TICK-2), в сцене без пары
 * полей точки — ошибка ЗАГРУЗКИ, называющая способность, шаг и недостающие
 * поля (ABIL-5, ABIL-10). Иначе платформа подставляла бы позицию владельца:
 * направление считалось бы от владельца к владельцу, то есть нулевое, и
 * способность стреляла бы «в никуда» каждый раз без единого сообщения.
 *
 * Живёт здесь, рядом с `hasAimPoint`, а не в разборе шагов: вопрос «есть ли у
 * сцены точка прицела» — вопрос биндингов, и второго ответа на него не бывает.
 */
export function requireAimPoint(bindings: CompiledBindings, kind: string, path: string): void {
  if (bindings.hasAimPoint) return;
  fail(
    path,
    `шаг вида "${kind}" берёт точку прицела ввода (ABIL-5, TICK-2), а компонент ввода ` +
      `"${bindings.inputComponent}" сцены не объявляет полей "${INPUT_TARGET_X_FIELD}" и ` +
      `"${INPUT_TARGET_Y_FIELD}" — платформа подставила бы позицию владельца, и шаг целился бы в никуда молча`,
  );
}
