 ...
 *
 * TimeScale (TIME-5): игнорирует — `phaseTicks` растёт на единицу за ГЛОБАЛЬНЫЙ
 * тик, как и прочие счётчики платформы (design Decision 9, тот же выбор, что у
 * кулдаунов и локомоушена). Следствие для дизайна: «скорость каста» статом этой
 * платформы не выражается — замедленный герой кастует ровно те же тики.
 */
...
export class CastPhaseSystem implements System {
  ...
  private interrupted(
    ...
    // и в них источник не распознаётся вовсе (ABIL-6). Это ЕДИНСТВЕННОЕ
    // исключение, названное требованием; упоминания источника в `interrupts`
    // распознавание не требует — определение переопределяет только отличия, а
    // платформенное умолчание («кулдаун целиком, прицеливание сбросить»)
    // действует на всё, чего оно не назвало. Данных у источника нет никаких, в
    // отличие от `disable` (маска лока) и `displacement` (предикат), и потому
    // «замка» по определению у него быть не может.
    //
    // Отпускание, которым игрок открывает прицеливание, срывом не становится:
    // спад бита виден ровно на одном тике (ABIL-3, TICK-4), а на этом тике слот
    // ещё в фазе `hold`/`release`, которая его потребляет, — переход в
    // следующую фазу случается позже в этом же прогоне.
    if (
