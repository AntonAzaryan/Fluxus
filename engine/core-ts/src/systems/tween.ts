 ...
 *
 * Разложение надвое — прямо по TWEEN-1, а не отступление от него: иммутабельная
 * таблица определений в конфиге сцены (`TweenDef { target, onComplete? }`,
 * SER-7) плюс компонент `Tween` из скалярных полей, где `def` — индекс
 * определения в этой таблице. Основание там же названо: поля ECS-компонента —
 * только скаляры `i32`/`fixed` (ECS-3, `ComponentSchema` в `types.ts`), поэтому
 * ни строка `"Health.value"`, ни дерево action'ов в компонент не помещаются.
 * `target` остаётся путём к полю (TWEEN-3), `onComplete` — действием,
 * исполняемым по завершении (TWEEN-4). Побочная выгода — путь к полю
 * разбирается один раз при сборке сцены, а не на каждом тике каждого твина.
 *
 * Таблица иммутабельна в течение матча, а ссылка `def` за её пределы — ошибка,
 * а не молчаливо пропущенный твин (TWEEN-1).
 *
 * TimeScale (TIME-5): учитывает или игнорирует — решает каждый твин сам
 * (TWEEN-7, TIME-4); см. поле `ignoreTimeScale`. Требование документировать
...
/**
 * Кривая: прогресс `[0, 1]` → доля пути `[0, 1]`. Индекс в таблице — это и есть
 * значение поля `easing`, поэтому запись `instant` здесь обязательна: без неё
 * константа `EASING_INSTANT` перестала бы быть известным easing'ом и падала бы
 * в `step` (TWEEN-2). Вычисляется она при этом никогда — скачок даёт быстрый
 * путь `done` в `step`, и он сильнее кривой: `instant` завершает твин целиком
 * (значение `to`, `onComplete`, снятие компонента), а не только отдаёт единицу
 * доли пути.
 */
