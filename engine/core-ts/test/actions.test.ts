...
describe('упорядоченная выборка forEach (ACT-5)', () => {
  ...
  /**
   * ACT-5: «расстояние SHALL считаться той же арифметикой квадратов и до той же
   * позиции, что у `withinRadius` (QUERY-1)». Смещение поэтому считается
   * обычной разностью языка, как в `ecs/query.ts`, а не через Math API:
   * `sub` заворачивает по правилам fixed-point (FP-4), и разность
   * `INT32_MAX − INT32_MIN` схлопнулась бы в −1, то есть самая дальняя цель
   * оказалась бы самой близкой.
   */
  it('смещение не заворачивается: порядок тот же, что у withinRadius (ACT-5, QUERY-1)', () => {
    const h = harness();
    const INT32_MAX = 2147483647;
    const INT32_MIN = -2147483648;
    // От центра `INT32_MIN` дальняя цель отстоит на 4294967295, ближняя — на
    // 2147483648. Завёрнутая разность дала бы им 1 и 2147483648, то есть
    // ровно обратный порядок.
    const far = spawn(h.world, 'Hero', { Position: { x: INT32_MAX, y: 0 } });
    const near = spawn(h.world, 'Hero', { Position: { x: 0, y: 0 } });

    execute([visit({ nearestTo: { vec: [INT32_MIN, 0] } })], h.ctx);
...
describe('источники-модификаторы из DSL (ACT-1, TIME-8)', () => {
  ...
  /**
   * TIME-8: постановка на сущность, которая самого списка не несёт, — ошибка, а
   * не пустая операция. Иначе запись ушла бы в хранилище, произведение
   * источников её не увидело бы, и замедления не случилось бы молча. Платформа
   * баффов в том же случае падает громко (`abilities/buffs.ts`), и двум путям к
   * одному и тому же расходиться нечем.
   */
  it('постановка на сущность без компонента-списка — ошибка (TIME-8)', () => {
    const h = harness();
    // `Projectile` объявлен без списков источников.
    const projectile = spawn(h.world, 'Projectile');
    expect(() =>
      { execute([{ addModifier: { entity: projectile, component: SLOW.component, id: 1, value: F(0.5) } }], h.ctx); },
    ).toThrow(/SlowSources: сущность \d+ не несёт этого списка источников \(TIME-8\)/);

    // Снятие отсутствующего источника ошибкой не бывает (TIME-8) — в том числе
    // у сущности без списка: система-владелец не обязана знать, что там.
    expect(() =>
      { execute([{ removeModifier: { entity: projectile, component: SLOW.component, id: 1 } }], h.ctx); },
    ).not.toThrow();
  });
