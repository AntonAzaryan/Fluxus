...
describe('NAV-3: данные производны от ассета и матчем не трогаются', () => {
  ...
  it('SER-5: поля "cost" у блока навигации не существует', () => {
  ...
  it('состав блока проверяется раньше, чем наличие террейна', () => {
    // Опечатка автора документа обязана назвать себя первой: отсутствие
    // террейна — свойство сцены (NAV-3), и сообщать о нём раньше значило бы
    // прятать неизвестное поле за чужой находкой.
    const { terrain: _terrain, ...flat } = scene;
    expect(() =>
      buildSimulation(
        {
          scene: flat,
