...
describe('Дисциплина аллокаций платформы NPC (NPC-4)', () => {
   ...
   * запросов к миру за тик — обоих каналов, `query` и `queryInto` (SYS-10).
   * Каждый запрос — единственная выборка, которую система снимает на тик; всё
   * прочее (сетка соседей, индекс маршрутов, кадр агента, спецификации
   * запросов, буферы выборок) живёт на экземпляре системы. Постоянное число
   * запросов при десятикратном росте числа агентов и означает «тик не
    ...
    run(ctx: SystemContext): void {
      const probe: SystemContext = {
        ...
        queryInto: (spec, ids, indices) => {
          const matched = ctx.queryInto(spec, ids, indices);
          // Считается ОТДАННЫЙ результат: проба, не поместившаяся в буфер
          // системы, — это её рост, а не второй запрос за тик (тем же правилом
          // считает счётчик трейса, DIAG-3).
          if (matched <= Math.min(ids.length, indices.length)) this.queries += 1;
          return matched;
        },
      };
  ...
  it('число запросов к миру за тик не зависит от числа агентов', () => {
    const small = queriesFor(4);
    // Ноль сравнялся бы с нулём и при потерянном счётчике: проверка имеет смысл
    // только если запросы вообще считаются.
    expect(small).toBeGreaterThan(0);
    expect(queriesFor(40)).toBe(small);
  });
});
