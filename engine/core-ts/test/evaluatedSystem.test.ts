...
describe('валидация на регистрации (SYS-3)', () => {
  ...
  /**
   * Ключ вне конвенции имён (SYS-3) отвергается, а не пропускается: опечатка в
   * имени НЕОБЯЗАТЕЛЬНОГО аргумента иначе значила бы «аргумент не задан» и
   * дожила бы до середины матча, а выражение внутри такого ключа не было бы
   * проверено на регистрации вовсе.
   */
  it('ловит аргумент вне конвенции имён и опечатку в имени необязательного аргумента', () => {
    expect(invalid({ do: [{ destroyEntity: { entity: v('e'), reason: v('e') } }] })).toThrow(
      /аргумент "reason" вне конвенции имён/,
    );
    expect(
      invalid({ do: [{ forEach: { query: { all: ['Health'] }, as: 'x', limitt: F(1), do: [] } }] }),
    ).toThrow(/аргумент "limitt" вне конвенции имён/);
  });

  it('ловит фильтр вне состава запроса (QUERY-1)', () => {
    expect(invalid({ query: { all: ['Health'], withinRadiuss: {} } })).toThrow(
      /фильтр "withinRadiuss" не входит в состав запроса/,
    );
  });
