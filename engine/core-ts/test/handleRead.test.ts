 ...
 * поведение. И теста «через handle нельзя писать»: изменяемой ссылки на колонку
 * не существует по построению — в контракте нет ни одного метода, который отдал
 * бы её (ниже это закреплено перечнем поверхности, а не попыткой записи).
 * Записать поле по handle можно КОМАНДОЙ буфера, и это тот же отложенный канал
 * CMD-1 — его проверяет `handleCommands.test.ts`.
 */
...
describe('handle-путь внутри тика (SYS-5, SYS-10)', () => {
  ...
  it('канала записи по handle в контракте нет (TICK-3, CMD-1)', () => {
    ...
    // Ровно четыре: два разрешения и два ЧТЕНИЯ. Ни записи прямо в контексте,
    // ни выдачи колонки — мутации идут только Command Buffer'ом, и запись по
    // handle там такая же отложенная команда, как строковая (CMD-1).
