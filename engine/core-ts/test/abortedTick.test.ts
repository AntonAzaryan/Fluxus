 ...
 *
 * Норма сегодня выполняется устройством `tick()` — буфер команд очищается на
 * ВХОДЕ в тик и потому не доносит накопленное упавшей системой до следующего
 * вызова, — и потому её легко потерять рефакторингом, не заметив: тесты
 * состояния мира после успешного тика такую потерю не увидят. Здесь она
 * пришпилена.
 */
...
describe('обрыв тика на системе посреди прохода (SYS-9)', () => {
  ...
  it('команды упавшей системы не доживают до следующего тика (SYS-9)', () => {
    const { sim, state, entity } = makeSim();
    tick(sim, state); // тик 1 проходит целиком
    ...
    // Буфер живёт дольше одного тика (ради своих массивов), и очистка на входе
    // — это то, чем держится пост-условие: доживи накопленное `Third` до тика
    // 3, его применил бы flush ПЕРВОЙ системы этого тика, и след упавшей
    // системы оказался бы в мире задним числом.
    expect(marks(state, entity)).toEqual({
      first: F(3), // слита на каждом из трёх тиков
      second: F(3),
      third: F(1), // след тика 1; команды тиков 2 и 3 отброшены целиком
      fourth: F(1),
    });
  });
