...
/**
 * Вспышка тинта на событие доставки (ASSET-18, `rendering` REND-40): «получил
 * урон — мигнул красным». Цвет и сила — то же, чем описан постоянный тинт;
 * длительность отличает вспышку от него: по её истечении инстанс возвращается
 * к тинту, который на нём стоял, а не к «без тинта».
 */
export interface TintFlash {
  /** Цвет вспышки, "#rrggbb". */
  color: string;
  /** Сила вспышки в [0..1]; без поля — 1: событие мигает в полную. */
  strength?: number;
  /** Полная длительность вспышки в секундах, больше нуля. */
  seconds: number;
}

/**
 * Блок тинта записи манифеста (ASSET-18, `rendering` REND-40): МАСКА
 * команд-цвета и таблица вспышек по событиям.
 *
 * Маска названа индексами материалов модели, а не каналом текстуры, по одному
 * основанию: индекс материала одинаково известен обоим ярусам представления
 * (`rendering` REND-20) — батч держит по материалу модели свой материал батча,
 * детальный инстанс — свою копию того же материала, — и тинт по маске
 * рисуется на них ОДИНАКОВО. Канал текстуры такого свойства не имеет: маской
 * по текселям батчевый ярус красил бы иначе, чем детальный, и один и тот же
 * контент выглядел бы по-разному в зависимости от пресета качества (QUAL-2).
 *
 * Так же устроены и модели: команд-цвет в них — отдельный материал (плащ,
 * наплечники), а не канал общей карты.
 */
export interface VisualTint {
  /**
   * Индексы материалов модели, на которые действует тинт. Без поля — тинт
   * действует на весь инстанс; пустой массив — не действует ни на что (запись
   * тинт объявила и сама же его выключила, и это законно).
   */
  materials?: number[];
  /** Имя события доставки → вспышка (REND-40); без таблицы вспышек нет. */
  byEvent?: Record<string, TintFlash>;
}

/**
 * Растворение трупа (`rendering` REND-4): сколько ждать после фиксации
 * последнего кадра клипа смерти и за сколько раствориться.
 *
 * Задержка — АВТОРСКОЕ число, а не длина клипа: длину клипа рендер знает, но
 * «сколько труп лежит» — художественное решение, и мерить его концом
 * анимации значило бы отдать его аниматору. Без блока труп не растворяется
 * вовсе — прежнее поведение сцены, снимающей `Dead` своей системой.
 */
export interface VisualDissolve {
  /** Секунды от фиксации смерти до начала растворения; без поля — 0. */
  delay?: number;
  /** Секунды самого растворения, больше нуля. */
  duration: number;
}
...
export interface EntityVisual {
  ...
  light?: VisualLight;
  /**
   * Тинт инстансов записи (ASSET-18, `rendering` REND-40): маска команд-цвета и
   * вспышки по событиям. Без блока инстансы записи тинта не получают вовсе, и
   * порт «цвет на сущность» на них не действует — маска пустая по построению.
   */
  tint?: VisualTint;
  /**
   * Растворение трупа (`rendering` REND-4). Без блока труп лежит, пока его не
   * снимет сцена, — прежнее поведение.
   */
  dissolve?: VisualDissolve;
  /**
   * Высота мирового якоря над позой инстанса, мировые единицы (`rendering`
   * REND-41): где висит полоска здоровья и имя над юнитом. Без поля — верх
   * габаритов инстанса: у модели, чьи габариты описывают её саму, это и есть
   * «над макушкой», а число нужно там, где макушка не там, где голова, — у
   * летящего плаща, у постройки с башенкой, у растянутого по земле босса.
   */
  anchorHeight?: number;
}
