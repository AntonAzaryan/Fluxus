 ...
 * HUD-8): окно значений стата и множители радиуса на его концах.
 *
 * Заведено ради телеграфа, который РАСТЁТ: шар заряда каста появляется перед
 * кастером и растёт вместе с зарядом — без роста игрок не видит, чем именно
 * сейчас выстрелят. Величина заряда приезжает обычным доставляемым статом, то
 * есть тем же входом, которым живёт вся подсистема эффектов; недостающим был
 * словарь записи, а не механизм, — потому это данные, а не модуль игровой
 * сборки. Стата в доставленном состоянии нет — оболочка рисуется числами
 * записи без ведения (REND-23).
 */
export interface EffectStatWindow {
  /** Имя доставленного стата (HUD-8). */
  stat: string;
  /** Значение стата в начале окна; без него — 0. */
  min?: number;
  /** Значение стата в конце окна; строго больше начала. */
  max: number;
  /** Множитель радиуса в начале окна; без него — 1. */
  from?: number;
  /** Множитель радиуса в конце окна. */
  to: number;
}

/**
 * Второй цвет эффекта с названной фазы окна (REND-23) — телеграф ПОРОГА: с этой
 * доли окна `radiusFromStat` шар заряда берёт цвет тяжёлого снаряда, того
 * самого, который родится на выстреле. Порог, а не плавный переход: игроку
 * сообщается факт «теперь улетит другое», а не близость к нему.
 */
export interface EffectColorAt {
  /** Доля окна [0..1], с которой берётся второй цвет. */
  phase: number;
...
/**
 * Мигание эффекта ЗА концом окна `radiusFromStat` (REND-23): предупреждение о
 * передержке. Часы — часы презентации подсистемы (REND-25): в стоящем мире
 * мигание замирает вместе с ними.
 */
export interface EffectBlink {
  /** Период полного цикла, мс. */
  periodMs: number;
  /** Множитель альфы в тёмной половине цикла, [0..1]. */
  alpha: number;
}
 ...
 *
 * Состав записи ЗАКРЫТ (REND-23): неизвестный КЛЮЧ отвергается валидацией
 * адресно, а не игнорируется молча.
 */
export interface VisualEffect {
  ...
  color: string;
  /**
   * Радиус в начале жизни, мировые единицы. Обязателен у РАДИАЛЬНЫХ примитивов;
   * нерадиальный (луч, лента, полоса) называет вместо него ширину, и радиуса у
   * него нет вовсе (REND-23).
   */
  radius?: number;
  ...
  /**
   * Вынос эффекта-оболочки вперёд по курсу прицела сущности, мировые единицы
   * (REND-23). Курс — ДОСТАВЛЕННЫЙ (`EntityView.aimYaw`, а без него курс
   * движения): локальный сэмпл ввода текущего кадра — вход единственной
   * подсистемы превью каста (REND-1, REND-28), и оболочке он не положен.
   * Отрицательное значение выносит назад.
   */
  offset?: number;
  /** Ведение радиуса доставленным статом (REND-23); без него радиус постоянен. */
  radiusFromStat?: EffectStatWindow;
  /** Второй цвет с названной фазы окна `radiusFromStat`. */
  colorAt?: EffectColorAt;
  /**
   * Периодическая модуляция альфы (REND-23). С окном `radiusFromStat` мигает
   * ЗА его концом — предупреждение о передержке; без окна пульсирует всегда —
   * так живёт луч и лента.
   */
  blink?: EffectBlink;
  /**
   * Внутренний радиус наземного кольца (REND-43), мировые единицы; без него
   * фигура сплошная. Радиальным примитивам, у которых дырки не бывает, поле
   * смысла не имеет.
   */
  innerRadius?: number;
  /**
   * Полураствор наземного сектора в ГРАДУСАХ (REND-43). Градусы, а не доля
   * оборота ядра (FP-7): манифест — presentation-данные, и пишет их автор
   * картинки, а не автор определения способности.
   */
  halfAngleDeg?: number;
  /** Длина наземной полосы вдоль направления сущности, мировые единицы. */
  length?: number;
  /**
   * Ширина полосы, луча и ленты, мировые единицы. Её наличие и означает, что
   * примитив НЕ радиальный: `radius` такой записи не требуется.
   */
  width?: number;
  /**
   * Мягкость кромки наземной фигуры, доля её размера в [0..1] (REND-43): 0 —
   * резкая кромка, 1 — спад от самого центра.
   */
  edgeSoftness?: number;
  /**
   * Подъём наземной фигуры над полем, мировые единицы (REND-43). Совпадение с
   * полом даёт z-fighting, а не изображение; без поля — документированное
   * умолчание рендера.
   */
  lift?: number;
  /**
   * Имя доставленного стата с сущностью ЦЕЛИ луча (REND-23, `match-hud` HUD-8).
   * Цели нет в доставленном состоянии — луч в этом кадре не рисуется, и
   * невидимую сущность он не открывает (`netcode` NET-12).
   */
  targetFromStat?: string;
  /**
   * Длина следа-ленты в выборках поз (REND-23). Фиксируется при взятии узла —
   * она и есть длина истории; без неё — умолчание рендера.
   */
  trailSamples?: number;
}
