...
export interface NormalizedModel {
  ...
  /**
   * Цепочка упрощённых уровней детализации, от грубого к грубейшему (ASSET-12,
   * `rendering` REND-22): уровень `k` — тот же набор частей, что `meshes`, но с
   * упрощённой геометрией. Уровень модели как она есть в цепочку НЕ входит: он
   * всегда нулевой, и дублировать его значило бы завести второй ответ на вопрос
   * «чем нарисован ближний инстанс».
   *
   * Поле НЕОБЯЗАТЕЛЬНО и заполняется только форматом, который цепочку несёт:
   * упрощать геометрию сам модуль ассетов не берётся — децимация это авторская
   * работа, а не производная (REND-22: модель без цепочки рисуется единственным
   * уровнем, и это прежнее поведение, а не деградация).
   */
  readonly lodMeshes?: readonly (readonly NormalizedMesh[])[];
}
