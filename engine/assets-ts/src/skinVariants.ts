/**
 * Набор вариантов скинов записи манифеста (ASSET-11, четвёртая производная):
 ...
 * `DataArrayTexture`. Запекание детерминировано (ASSET-11): ресайз — выборка
...
/** Запечённый набор вариантов записи (ASSET-11). */
...
/**
 * Запекает набор вариантов (ASSET-11). Первый вариант — базовый: он задаёт
...
export function bakeSkinVariants(
  ...
  // порядок обхода не должен зависеть от порядка вставки в Map (ASSET-11).
  const slotNumbers = [
 ...
 * а не усреднение: детерминизм запекания (ASSET-11) требует правила, которое
