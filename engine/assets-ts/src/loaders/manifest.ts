 ...
 * ровно тогда секция эффектов проверяется по нему (ASSET-8). Тем же путём
 * приезжает описание конфига камеры (CAM-1) для секции конфига (ASSET-10).
 * Предупреждения логируются и загрузку не роняют: манифест переживает код.
 */
...
export interface ManifestLoaderOptions {
  ...
  /**
   * Машинное описание конфига камеры (CAM-1) — тем же путём и по той же
   * причине, что описание типов эффектов: перечень параметров живёт в коде
   * камеры (ASSET-10). Нет — секция конфига проверяется структурно.
   */
