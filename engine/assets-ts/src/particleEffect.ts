...
/**
 * Суффикс имени эмиттерного ассета. Отдельный суффикс, а не просто `.json`:
 * реестр различает форматы парой «вид + расширение» (ASSET-3), а вот АВТОРУ и
 * инструментам дерева контента документ эффекта надо отличать от прочего JSON
 * по имени файла — как парный presentation-документ отличается своим
 * (`presentation-scene` PRES-1).
 */
export const PARTICLE_EFFECT_SUFFIX = '.effect.json';

/** Эмиттерный ли это ассет по имени файла (ASSET-14). */
export function isParticleEffectPath(path: string): boolean {
  return path.endsWith(PARTICLE_EFFECT_SUFFIX);
}
