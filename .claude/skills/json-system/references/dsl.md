...
Имена аргументов — закрытая конвенция (SYS-3): ключ, которого в ней нет, отвергается на регистрации системы. Опечатка в имени НЕОБЯЗАТЕЛЬНОГО аргумента (`radiuss`, `limitt`) — поэтому ошибка загрузки, а не молчаливое «аргумент не задан»; то же и для ключа внутри `query`, состав фильтров которого закрыт (QUERY-1).
