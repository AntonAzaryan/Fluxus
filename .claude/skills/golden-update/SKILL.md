...
1. **Поведение менялось намеренно и по спеке?** Сначала ответь на этот вопрос. Изменение должно соответствовать требованию в `engine/openspec/specs/` (или идущему change'у). Если нет — это регрессия: чини код, эталоны не трогай.
