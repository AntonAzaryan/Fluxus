...
Требования, проектная документация и реализация движка. Спека — источник правды о том,
каким движок должен быть; пакеты `*-ts` — его текущая рабочая реализация.

### Спека и документация

- `openspec/specs/` — **нормативные требования**, разложенные по capability
  (DET-, ECS-, NET-, NTR-, NAV-, FOW- …). Список и объём — `openspec list --specs`
