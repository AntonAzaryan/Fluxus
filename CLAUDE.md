...
A game app is not engine, and the line matters mechanically: `game/demo-ts` reads `content/` legally — it *is* the game — while an engine package must not, so that the content tree can be absent and `npm run check` still passes (CONT-4). The barrier is the depcruise rule `engine-no-content` (`npm run lint:arch`).
