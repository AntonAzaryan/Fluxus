...
A game app is not engine, and the line matters mechanically: `game/demo-ts` reads `content/` legally — it *is* the game — while an engine package must not (CONT-4). The barrier is the depcruise rule `engine-no-content` (`npm run lint:arch`), and it sees module-graph edges only: a static `import` of `content/**` from `engine/**` is an error, an `fs` read of the same path is invisible to it. So "the content tree can be absent and `npm run check` still passes" is the goal, not the current state — `engine/client-ts` reached it with this move, and the demo-arena stand followed it out (`game/demo-ts/bin/demo-serve.mjs` — it reads `content/` as the game, legally, through `@game-mvp/net`'s published surface), while two places under `engine/` still read the tree through `readFileSync` and break without it: `engine/bot-ts/test/profile.test.ts` (the reference bot profiles) and `engine/integration-ts/test/presentationIsolation.test.ts` (the demo layer of the real scene). Both reads are deliberate and explained in place; they are the remaining CONT-4 debt, and adding a third is a step backwards.
...
`openspec/specs/` (35 capabilities, 420 requirements) normatively defines what the engine must be. When implementation and spec diverge, the defect is in the implementation (CORE-3). Normative statements live **only** in the specs — do not duplicate them in docs or code.
