...
`openspec/` is the spec, `engine/` its working implementation (the `*-ts` packages), `editor/` the content editor built on top of them (capability `editor`, roadmap stage 12), `game/` the game applications built on the engine (today one — `game/demo-ts`, the demo arena, the demo consumer of capability `client-shell`), `tools/` the Blender level-design pipeline (capability `blender-pipeline`, roadmap stage 35), `desktop/` the Electron desktop container for the editor and the game client (capability `desktop-shell`, roadmap stage 38), `content/` the game content the engine runs, `docs/` the non-normative overview. The npm workspace root is the repository root.

A game app is not engine, and the line matters mechanically: `game/demo-ts` reads `content/` legally — it *is* the game — while an engine package must not (CONT-4). The barrier is the depcruise rule `engine-no-content` (`npm run lint:arch`), and it sees module-graph edges only: a static `import` of `content/**` from `engine/**` is an error, an `fs` read of the same path is invisible to it. So "the content tree can be absent and `npm run check` still passes" is the goal, not the current state — `engine/client-ts` reached it with this move, while two engine tests still read the tree through `readFileSync` and go red without it: `engine/bot-ts/test/profile.test.ts` (the reference bot profiles) and `engine/integration-ts/test/presentationIsolation.test.ts` (the demo layer of the real scene). Both reads are deliberate and explained in place; they are the remaining CONT-4 debt, and adding a third is a step backwards.
...
`openspec/specs/` (30 capabilities, 362 requirements) normatively defines what the engine must be. When implementation and spec diverge, the defect is in the implementation (CORE-3). Normative statements live **only** in the specs — do not duplicate them in docs or code.
...
Node >= 22.18. The repository root is the npm workspace; its members are the engine packages (`engine/core-ts`, `engine/net-ts`, `engine/render-ts`, `engine/assets-ts`, `engine/client-ts` — the web client shell (worker hosting + channel, SHELL-1..8), `engine/hud-ts` — the HUD widgets (capability `match-hud`), `engine/bot-ts` — the bot player (capability `bot-player`), `engine/integration-ts` — the cross-layer integration suite, CLI-9) plus the two editor packages (`editor/core-ts` — the headless authoring layer, `editor/ui-ts` — its DOM interface and web app), the demo game app (`game/demo-ts` — the arena built on the client shell; its web app lives under `app/`, and its tests read `content/` legally), the Blender importer (`tools/blender-ts` — glTF parsing, spatial-layer generation and the `import` CLI; the `tools/blender-addon/` add-on is Python and is **not** a workspace member) and the desktop container (`desktop/shell-ts` — the Electron shell hosting the editor and the game client, capability `desktop-shell` DSK-1..6; Electron itself stays outside the `check` gate). A single `npm install` from the root installs everything, and the check-everything gate before pushing is `npm run check` — it is `typecheck && lint && spec-graph check && test`, and running only a subset is how a lint or spec-graph failure reaches the remote:
...
The demo game runs from the root as `npm run demo` (= `npm run dev -w @game-mvp/demo`; Vite serves `content/` as the content tree, `game/demo-ts/app/vite.config.ts`). The editor's web app runs from the root as `npm run dev -w @game-mvp/editor-ui` (Vite; `app/vite.config.ts` serves `content/` as the content tree and adds the tree-listing endpoint the web environment host needs — a `vite build` has neither listing nor write, and says so, ED-12).
