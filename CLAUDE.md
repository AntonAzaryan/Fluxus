...
npm run textures:ground # same for the terrain tileset textures (content/visuals/textures/ground-*.png — grass,
                        # dirt, rock and the cliff wall) from scripts/gen-ground-textures.mjs; which of them a scene
                        # actually wears is the manifest's `terrain.tileset`, not the script (ASSET-15, REND-39)
```
