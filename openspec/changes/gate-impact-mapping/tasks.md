...
- [ ] 1.3 Инвариант стадий (CLI-13) — в исполнителе плана (2.3), не отдельным тестом (тест, гоняющий гейт, был бы рекурсией): `scripts/impact.mjs` снимает `git status --porcelain` до и после исполнения любого плана и краснит с перечнем файлов, если стадия изменила отслеживаемое дерево; проверка — юнит-тест сравнения снимков, `npm run check:timings` на чистом дереве проходит без находки
