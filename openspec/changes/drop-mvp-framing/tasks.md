...
- [ ] 4.1 `grep -rn "MVP" openspec/specs/ openspec/config.yaml` → пусто; `grep -rn "MVP" openspec/changes/ --exclude-dir=archive` → только дельты и артефакты этого change'а
- [ ] 4.2 `openspec validate --specs --strict` и `npm run spec-graph -- check` зелёные; ни один ID не потерян — сверить список `CORE-1 CORE-4 DI-4 NTR-6 NTR-9 NTR-10 NTR-12 NTR-14 NTR-17 NET-8 NET-18 FOW-2 FOW-7 FOW-9 REND-7 REND-8 SNAP-4 EXPR-1 SES-5 SHELL-6` через `npm run spec-graph -- show <ID>`
- [ ] 4.3 `npm run check` из корня зелёный (правок кода change не несёт — красным гейт стать не может; прогон подтверждает, что `spec-graph check` не сломан переименованием заголовков)
- [ ] 4.4 Сверить блок «Неснимаемые принципы» в `openspec/config.yaml` с одноимённым разделом `CLAUDE.md` — формулировка про WASM должна совпадать в обоих
