...
- [x] 0.1 Гейт синхронизации, а не реализации: на сегодня `editor-viewport-ergonomics` в главную спеку НЕ синхронизирован (последний номер `camera` — `CAM-7`), поэтому галочка снята до архивации. Убедиться, что `editor-viewport-ergonomics` синхронизирован в главную спеку раньше этого change: он занимает `CAM-8`, здесь взят `CAM-9` (`proposal.md`, «Порядок в очереди»). Если порядок перевернулся — переименовать требование обратно в `CAM-8` во всех файлах этого change'а и перевести `editor-viewport-ergonomics` на `CAM-9`; дырой в нумерации `camera` оставлять нельзя
  - Гейт исполнен: `editor-viewport-ergonomics` заархивирован первым (2026-08-09), `CAM-8 Кадрирование по заданным границам` стоит в главной спеке `camera`, и `CAM-9` этого change'а лёг следующим номером без дыры. Переименовывать ничего не потребовалось.
...
- [x] 0.3 После синхронизации этого change'а перестроить дельту `ASSET-8` в `dedup-normative-duplicates` по слитому тексту — тем же приёмом, каким `terminology-sweep` перестроен по слитому `REND-1`
  - Сделано сразу после архивации: дельта `ASSET-8` в `dedup-normative-duplicates` построена заново по слитому тексту главной спеки (первый абзац со ссылкой на CAM-9, оба абзаца про валидацию по описанию и три новых сценария сохранены), а собственной правкой dedup в ней остался ровно второй абзац — перечень последствий заменён ссылкой на `ASSET-1`. Записано в его `proposal.md` (Impact) и `tasks.md` (задача 1.3).
- [x] 0.4 Координация по файлам с соседями по очереди: `editor/ui-ts/src/areas/sceneProject.ts` и `editor/ui-ts/app/assembly.ts` правит и `editor-authoring-areas` (его WP8) — регистрация правил движка (задача 5.3) и регистрация двух новых областей сходятся в этих двух файлах
  - Обе правки сошлись без конфликта: `sceneProject.ts` несёт `engineValidationRules(..., { cameraEffects: CAMERA_EFFECTS_DESCRIPTION })`, `assembly.ts` — `registerCameraEffectsOperations` поверх `registerVisualsOperations` и обе новые области (`createObjectsArea`, `createSystemsArea`).
...
- [ ] 10.3 Прогнать редактор глазами: `npm run dev -w @game-mvp/editor-ui`, завести тряску от взрыва в таблице событий и убедиться, что на диск уходит валидная секция (ED-14, сценарий «Тряска от взрыва без правки JSON»)
  - Не сделано и при архивации не закрывается: живой прогон браузером этого прохода проверял вьюпорт (`editor-viewport-ergonomics`, задача 6.3), таблицы секции эффектов в нём не открывались. Автоматикой путь закрыт с двух концов — обратимость операций и отказы на незаявленный параметр и значение вне диапазона (задача 7.6), появление типа из подставленного описания в таблице (8.5), — но «на диск уходит валидная секция» глазами не подтверждено. Остаётся работой следующего прохода.
