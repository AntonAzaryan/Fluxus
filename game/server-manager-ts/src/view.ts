...
function detailsPanel(details: ServerDetails): UiNode {
  ...
  // уже нет, кнопки недоступны, а не отказывают после нажатия. Сами детали при
  // этом открыты — ради них в упавший сервер и кликают (SRV-6).
  const live = entry.state === 'listening' || entry.state === 'starting';
  return node('section', 'mg-details', [
    ...
    // Материалы разбора показываются РОВНО у того сервера, чей процесс ушёл: у
    // живого их нет, и строка «код выхода: —» ничего бы не сообщала.
    ...(live ? [] : [postmortemRow(entry)]),
