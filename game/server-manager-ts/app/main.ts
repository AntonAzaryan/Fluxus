...
// Политика завершения (MGR-4) живёт в `src/closePolicy.ts` — здесь только
// подписка на запрос закрытия: сам ответ проверяется тестом, а не глазами по
// закрывающемуся окну.
//
...
// страницы (с крайним сроком на его стороне).
const closeHook = bridge?.onCloseRequest;
if (closeHook !== undefined) {
  closeHook(() => closeManager(session, bridge));
} else {
