 ...
 * внутри бандла. А страница читает отсюда не только ассеты: документ матча и
 * конфиг сцены она берёт по ID-пути этой же раздачи (`game-content` CONT-5) и из
 * них выводит версию, которую предъявляет в рукопожатии, — поэтому заслонённое
 * дерево разошлось бы с сервером по хешу контент-пака (NTR-5) и молча не пустило
 * бы игрока в матч. Такой бандл — не конфигурация, а собранный не тем конфигом
 * артефакт, поэтому он НАЗЫВАЕТСЯ отказом на старте (`contentShadowedBy`), а не
 * молча раздаётся.
 *
