...
/**
 * Версии контента, которым агент играет (SRV-7). Выводятся из ДЕРЕВА, которое
 * агент раздаёт, а не из записи о сборке: дерево внутри дистрибутива одно, и
 * читают его и сервер матча, и страница игрока (CONT-5), — поэтому названная
 * здесь версия и есть та, на которой сойдётся рукопожатие (NET-16, NTR-5).
 */
