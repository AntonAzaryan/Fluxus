...
/**
 * Потолок ХВОСТА без перевода строки — общий, включая управляющие линии.
 *
 * `LOG_LINE_LIMIT` управляющую линию не режет намеренно (половина JSON перестаёт
 * быть отчётом), но исключение без потолка означает буфер, который растёт без
 * предела: стенд, начавший управляющую линию и не закончивший её (обрыв на
 * середине записи, сломавшийся отчёт), кончается не отказом, а `RangeError`
 * внутри обработчика `data` — то есть смертью агента вместе со ВСЕМИ его
 * серверами (SRV-1). Потолок взят с огромным запасом к самой большой законной
 * линии (отчёт со слотами — единицы килобайт), чтобы штатного отчёта он не
 * задевал никогда.
 */
export const CONTROL_TAIL_LIMIT = 1 << 20;
...
export function startStandProcess(options: StandProcessOptions): StandProcess {
  const child: ChildProcess = spawn(options.runtime, [options.script, ...options.args], {
    ...
    // СВОЯ группа процессов, и на обеих платформах. Иначе Ctrl+C в терминале
    // агента приходит всей группе (POSIX — SIGINT группе переднего плана;
    // Windows — CTRL_C_EVENT всем, кто сидит на той же консоли) и убивает все
    // идущие матчи разом, а серверы обязаны переживать уход агента: на этом
    // стоит и книга процессов (D5), и выключенный тумблер MGR-4.
    //
    // `detached` на Windows означает ровно это и ничего лишнего: libuv ставит
    // `DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP`, то есть консоли у ребёнка
    // нет вовсе — ни своей, ни нашей. Окна поэтому не появляется, но
    // `windowsHide` остаётся: он про то же и стоит копейки, а полагаться на
    // побочный эффект чужого флага в вопросе «не мигнёт ли окно» не стоит.
    // Один `detached` на обе платформы вместо ветвления: разница между ними
    // здесь — в механике ОС, а не в том, чего мы добиваемся.
    //
    // Останов от этого не страдает: он идёт по PID (`child.kill`, а для
    // пережившего агента — `stopProcessByPid`), а не сигналом группе, и на
    // Windows `kill` и так не сигнал, а `TerminateProcess` по хендлу процесса.
    // Ссылку в цикле событий `detached` не отпускает — для этого нужен `unref`,
    // которого здесь нет намеренно: пока агент жив, он ждёт своих детей.
    detached: true,
  ...
  const reader = (handle: (line: string) => void): ((chunk: string) => void) => {
    ...
    return (chunk: string): void => {
      ...
      // Но и у исключения есть потолок (`CONTROL_TAIL_LIMIT`): хвост, начатый
      // маркером и не получивший перевода строки, иначе растёт до `RangeError`.
      // Сбрасывается он В ЛОГ и НАЗВАННЫМ: молча выброшенный мегабайт выглядел
      // бы как «стенд перестал отчитываться» — то есть отлаживался бы не там.
      if (rest.length > CONTROL_TAIL_LIMIT) {
        rest = '';
        remember(
          `агент: управляющая линия длиннее ${String(CONTROL_TAIL_LIMIT)} знаков ` +
            'без перевода строки — хвост сброшен (SRV-1)',
        );
      }
    };
  };
