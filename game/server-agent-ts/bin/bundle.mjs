 ...
 * страница читает документ матча и конфиг сцены из той же раздачи, что и ассеты
 * (`game-content` CONT-5), и версию (NET-16, NTR-5) выводит из ПРОЧИТАННОГО.
 * Вторую версию контента внести в дистрибутив поэтому нечем — второго дерева в
 * нём нет.
 *
 * Страница игрока при этом либо СОБРАНА ЭТОЙ ЖЕ командой из этого же дерева
 * репозитория, либо её нет вовсе — третьего состояния у дистрибутива не бывает,
 * и это прямое следствие SRV-7. Основание тут не контент, а КОД: страница есть
 * код клиента, и его происхождение не покрыто ни одной половиной версии
 * рукопожатия. `buildId` — поле документа матча, которое страница прочтёт вместе
 * с остальным, поэтому страница чужой сборки предъявила бы версию ХОЗЯИНА и
 * прошла бы сверку, исполняя другие правила. Удостоверяет её происхождение
 * только сборка той же командой из того же дерева репозитория, что и сервер
 * матча.
 *
...
// Готовая страница со стороны — код клиента неизвестного происхождения (см.
// шапку). Отказ громкий и называет причину: молча сменившийся смысл флага дал бы
// старому скрипту тихо другое поведение.
if (option('client-dist', '') !== '') {
  console.error('флаг --client-dist больше не принимается: происхождение кода страницы не удостоверяет');
  console.error('ничто, кроме сборки вместе с дистрибутивом. Версии рукопожатия его не покрывают:');
  console.error('buildId — поле документа матча, который страница читает из раздачи (NET-16, NTR-5),');
  console.error('поэтому чужая сборка предъявила бы версию хозяина и играла бы по другим правилам (SRV-7).');
...
// 4. Запись о сборке: чем собрали, откуда страница и какие версии видел этот
//    прогон. Хеш контент-пака считается кодом, которым его считают сервер и
//    клиент (NET-17), а не переписывается руками, — но ЗАПИСЬЮ О РАЗДАВАЕМОМ он
//    не становится: версию контента агент выводит из дерева, которое раздаёт
//    (SRV-7, `agent.mjs`). Иначе правка дерева внутри дистрибутива дала бы третью
//    версию, которой нет ни у сервера, ни у страницы.
...
writeFileSync(
  ...
  '# Дистрибутив хоста Fluxus\n\n' +
    ...
    `node ${agentPath} pair\n` +
    '```\n\n' +
    `Версии дистрибутива на момент сборки: buildId \`${distribution.buildId}\`, ` +
    `контент-пак \`${distribution.contentPackHash}\`.\n\n` +
    'Дерево контента в дистрибутиве ОДНО, и читают его оба: сервер матча поднимает по\n' +
    'нему матч, а страница игрока берёт оттуда же документ матча и конфиг сцены — по\n' +
    'ID-пути той же раздачи, что и ассеты (CONT-5, SRV-8). Правка дерева поэтому\n' +
    'доезжает и до сервера, и до заново открытой страницы: версию (NET-16, NTR-5) обе\n' +
    'стороны выводят из прочитанного, и вход не отклоняется. Числа выше — запись о\n' +
    'том, ЧТО СОБРАЛИ; версию раздаваемого дерева агент называет менеджеру сам.\n\n' +
    'Пересборка (`npm run host:bundle`) нужна для новой сборки КОДА — агента, сервера\n' +
    'матча и страницы игрока: происхождение кода страницы удостоверяет только она\n' +
    '(SRV-7).\n',
);
