 ...
 * - `present` — `remote.frame(now)`: приём доставки подсистемами (`syncTick`,
 *   где живёт пересборка маски FoW) и покадровое обновление (`frame`), плюс
 *   фигуры главного потока, которые садятся на позу этого же кадра;
