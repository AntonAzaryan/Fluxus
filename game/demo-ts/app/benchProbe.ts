 ...
 *   (интерполяция поз, отсечение, выбор уровня детализации), плюс фигуры
 *   главного потока, которые садятся на позу этого же кадра;
 ...
 *
 * - приём доставки (`syncTick`, где живёт пересборка маски FoW) в кадр не
 *   попадает вовсе: подсистемам его раздаёт `RemoteHost` НА ПРИХОД сообщения
 *   из воркера (SHELL-3), то есть между кадрами, а не внутри rAF-колбэка;
