...
export function createDemoExtractor(grid: TerrainGrid | undefined): Extractor {
  return new Extractor({
    ...
    // Отталкивание в упор (`bossRepel`) — второй адрес доворота: босс обязан
    // РАЗВЕРНУТЬСЯ к подбежавшему, а курс инстанса производен от скорости
    // (REND-13) и стоящего босса не поворачивает. Событие несёт `dirX`/`dirY`
    // на жертву, и доворот корпуса с головой (REND-5) — единственный способ
    // показать разворот, не двигая сущность.
    aimEvents: ['CastFireball', 'BossRepelLanded'],
