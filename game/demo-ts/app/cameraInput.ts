 ...
 *
 * Второй — камера в follow (закреплена на герое). Мышь в этой сборке — орган
 * ПРИЦЕЛА: `ЛКМ` бьёт в точку под курсором, а захват и заряд каста целятся ею
 * всё время удержания (`main.ts`, `pointerAimSource`). Игрок, целящийся в
 * сторону кромки арены, уводит курсор в краевую полосу постоянно и панорамы при
 * этом не просит — а любой ввод панорамы в follow открепляет камеру (CAM-2), то
 * есть прицел в край срывал бы слежение за героем. Стрелки и drag средней
 * кнопкой открепляют по-прежнему: их двусмысленности нет, ими игрок именно
 * ведёт камеру. В free-RTS край работает как работал — там открепляться уже не
 * от чего, и панорама краем экрана остаётся основным способом вести камеру.
 */
