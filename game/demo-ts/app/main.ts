  ...
  remote = new RemoteHost(context, {
    ...
    onReady: (hello) => {
      ...
      const lighting = new LightingSubsystem({
        ...
        camera,
        // Рендерер — вход теневых проходов режима `hybrid` (REND-30): глубину
        // ярусов и их сведение в карту источника подсистема ведёт САМА, в своём
        // кадре, до отрисовки сцены. Без порта она исполняла бы `hybrid` как
        // `full`: картинка та же, а кэша статики нет — то есть балансный пресет
        // демо (потолок `hybrid`) платил бы за статику каждым кадром.
        renderer: renderer3,
