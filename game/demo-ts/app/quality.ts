 ...
 *
 * - `balanced` — умолчания движка плюс два потолка теней. Ручки, которым
 *   документ ставит их документированное умолчание (`models.lodThresholdScale`
 *   1, `models.defaultTier` `batched`, `particles.density` 1,
 *   `lighting.maxLocalLights` 4), написаны явно, а не пропущены: «здесь ничего
 *   не ограничено» — решение, и читаться оно должно в документе, а не
 *   выводиться из пустого объекта. Тени же пресет по умолчанию УДЕШЕВЛЯЕТ, и
 *   это его отличие от «сегодняшнего поведения»:
 *   - `lighting.shadowMode` `hybrid` при умолчании ручки `full` (REND-30).
 *     Сцена дуэли объявляет `full` (`content/scenes/duel.presentation.json`),
 *     семантика ручки — потолок, и действует `min(авторское, потолок)`
 *     (`LightingSubsystem.effective`): покадрово рисуется только динамический
 *     ярус, а документ сцены не правится ни байтом — та же семантика, что у
 *     потолка разрешения маски тумана (FOW-10);
 *   - `lighting.shadowMapSize` 1024 при авторских 2048 той же секции: сторона
 *     карты вдвое меньше, текселей вчетверо. Умолчание ручки — «потолка нет»
 *     (бесконечность), поэтому здесь это тоже ограничение, а не переписанное
 *     умолчание.
 *   Информации игроку ни то, ни другое не меняет (QUAL-2): это способ
 *   нарисовать тень, а не то, что игрок про мир знает.
 ...
 *   Пришедшие позже фичи добавили в этот же документ свои потолки — тени
 *   (`lighting.shadowMode` `blob`, `lighting.maxLocalLights` 2),
 *   пост-обработка (`postprocess.bloom`, `postprocess.lut` выключены) и вода
 *   (`water.rippleSources`, `water.detailLayers`, `water.depthTexelsPerCell`);
 *   перечень выше не исчерпывающий — исчерпывающим является сам документ.
 * - `ultra` — авторская картинка без потолков: теней документ не трогает вовсе,
 *   поэтому действует авторский режим сцены (`full` у дуэли) и авторская
 *   сторона карты. Единственное, что пресет поднимает выше умолчания движка, —
 *   `lighting.maxLocalLights` 8 при умолчании 4 (REND-33): ручка не потолок, а
 *   значение, и восьми источников арена вроде дуэльной стоит. Ярусов моделей
 *   это не касается: `models.defaultTier` остаётся `batched`, потому что
 *   детальный ярус по умолчанию сменил бы носителя тому, что демо показывает
 *   сейчас, — герой и так детальный (у записи `Hero` есть `boneControls`,
 *   REND-5), а ярус по умолчанию наследует ровно одна запись, декорация
 *   `Statue`. Ярус назовёт сам автор записи манифеста (`tier`, ASSET-13); числа
