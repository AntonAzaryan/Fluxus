...
/**
 * Маркер провала сцены демо (`FallStart` по событию `FellThroughFloor` ядра,
 * `arena` ARENA-5). Знание СБОРКИ той же природы, что `DEAD_COMPONENT`: провал
 * — конвенция ядра, а компонент, которым сцена помечает провалившегося, её
 * собственный. Состояние это ТЕРМИНАЛЬНОЕ: снимает `Falling` только возрождение
 * (`Respawn`), а до него герой летит вниз мимо арены и погибает по достижении
 * глубины (`FallDeath`).
 */
...
export const STATE_COMPONENTS: readonly string[] = Object.freeze([
  ...
  'Holding',
  'Charging',
  // Маркер смерти сцены (системы `HealthDeath`/`FallDeath`/`KillSwitch`). Едет
  // он ради фиксации клипа смерти по доставленному состоянию (REND-4): событие
  // `EntityDied` живёт один тик, а инстанс — сколько угодно, поэтому труп,
  // вернувшийся из тумана, и босс, возрождённый СВОЕЙ системой, без него
  // читались бы неверно (FOW-8).
  DEAD_COMPONENT,
]);

// Свободных битов у колонки `flags` больше нет: их шесть
// (`MAX_STATE_COMPONENTS` рендера), и все шесть заняты списком выше. Поэтому
// жгут вытягивания босса (`bossTether`) едет к рендеру НЕ состоянием, а
// доставленным статом жертвы (`STATS.tetherTarget`): луч между двумя
// сущностями рисует оболочка визуального ТИПА — запись `effects.byKind.Boss` с
// полем `targetFromStat` (REND-23). Седьмого состояния заводить не пришлось.
 ...
 * (запись `effects.byState.Charging` манифеста, REND-23) и порог, с которого
 * главный поток показывает предикт траектории (`main.ts`).
 *
...
export const STATS = {
   ...
   * заряда, то есть записи `effects.byState.Charging` манифеста (REND-23). Оба
   * живут НА ГЕРОЕ, а не на сущности-слоте, и это несущее: слот виден только
   * стороне владельца
   ...
   * кастера к названной им сущности. Обычный доставляемый стат (HUD-8), как и
   * цель движения агента: номер сущности, а не координаты, и невидимую жертву
   * он не открывает — не доставленную сущность клиент по номеру не разрешит, и
   * луч в этом кадре просто не рисуется (NET-12).
   */
  tetherTarget: 'tether',
