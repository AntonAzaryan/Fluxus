 ...
 *
 * `dodge` и `jump` слотов не имеют вовсе: это манёвры локомоушена (LOC-4,
 * LOC-5), а не способности платформы. Виджет их кнопки рисует, а стата не
 * находит и показывает «нет данных» — то есть кнопку без затемнения (HUD-8).
 */
