/**
 * Сессия прямо во вкладке (`net-session` SES-1, design D1): авторитетный цикл
 * матча исполняется в том же приложении, что и игрок, — но НЕ вторым путём
 ...
 *
 * Режим сессии у неё — `listen`, а не отдельный «локальный»: режим называет
 * ровно одну величину — где исполняется авторитетный цикл (SES-1), — и здесь он
 * исполняется в процессе игрока. То, что удалённых участников в этом матче
 * может не оказаться ни одного, — свойство ростера, а не размещения авторитета:
 * второй слот вкладки открыт заместителю ровно так же, как у стенда (NTR-18).
 * «Локальный» в имени этого файла — про ВКЛАДКУ (`demoMode`, `client-shell`
 * SHELL-8), а не про ось режимов сессии.
 *
 ...
 * `WorkerLike`.
 *
 * Чего здесь нет: ни одной ветки «если мы во вкладке, то…». Сервер о том, где
 * он живёт, не знает и знать не может — режим сессии до него не доходит
 * (SES-2).
 */
...
export class DemoLocalSession {
  ...
  constructor(options: LocalSessionOptions) {
    ...
    this.info = createSessionInfo('listen', 'in-page');
