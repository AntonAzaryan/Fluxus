...
describe('захват снаряда: удержание, переброс и смена владельца', () => {
  ...
  /**
   * Раствор — ГЕЙТ ВЫБОРА, а не пункт проверки на завершении фазы (ABIL-5).
   * Отсюда наблюдаемое следствие на пути «подтверждение отдельной кнопкой»
   * (геймпад, тач-оверлей — `bindings.json`): шаг, накопленный внутри конуса,
   * переживает отвод прицела, потому что проверка на подтверждении фигуру не
   * повторяет — она смотрит «жива, фильтр истинен, в дальности, видна».
   *
   * До переноса конуса в фигуру то же самое отсекал рукописный предикат
   * `filter`, который проверка ПОВТОРЯЕТ, — так что различие видно только здесь
   * и только на этом пути: на мыши отпускание и накапливает, и завершает фазу
   * одним тиком. Строгость этого рода остаётся выразимой предикатом.
   */
  it('накопленный шаг переживает отвод прицела: фигура — гейт выбора (ABIL-5)', () => {
    const a = arena(8);
    chargeA(a, 0);
    const shot = fireballs(a.state)[0]!;
    const p2 = a.heroes[1]!;
    ...
    // Кнопка захвата зажата: фаза `release` живёт, пока её держат.
    a.step(NEUTRAL, { buttons: CAPTURE, aimDir: AIM_WEST });
    // Подтверждение отдельным битом накапливает шаг, не завершая фазы.
    a.step(NEUTRAL, { buttons: CAPTURE | CONFIRM, aimDir: AIM_WEST });
    expect(coreWorld.hasComponent(a.state.world, shot, 'Held')).toBe(false);
    // Прицел уводится на противоположную сторону, и только потом кнопка
    // отпускается: накопленная цель остаётся накопленной.
...
describe('щит: рикошет снаряда, смена владельца и связки способностей', () => {
  ...
  it('щит не крадёт снаряд без отражения: перебор подклеточных выравниваний', () => {
    ...
    // Собственное окно: перебор поднимает 256 миров матча и на свободной машине
    // укладывается в ~3 с — то есть больше половины умолчания vitest (5 с). Под
    // общим прогоном `npm run check`, где тикают все пакеты сразу, он это
    // умолчание перешагивает, и гейт краснеет не от поведения, а от загрузки
    // машины. Число тут — запас над стоимостью перебора, а не бюджет: реальное
    // удорожание симуляции ловят счётчики стоимости и их голден (PERF-3/PERF-4).
  }, 30_000);
