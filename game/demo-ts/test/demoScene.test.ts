...
const RAMP_ROUTES: readonly RampRoute[] = [
  // Плато 11..18 × 11..18 (низкие ряды — ближний к камере край, на экране это
  // левый нижний угол): полоса рампы в колонке 19, вход с земли колонки 20.
  { name: 'СЗ', level: PLATEAU_LEVEL, path: [[8.5, 20.5], [20.5, 20.5], [20.5, 14.5], [17.5, 14.5]] },
  // Плато 29..36 × 29..36 (на экране — правый верхний угол): полоса рампы в
  // колонке 28, вход с земли колонки 27. Два других плато сняты — их место
  // заняло озеро в центре арены (парный presentation-документ, PRES-2).
...
describe('демо-сцена: подъём по рампам обоих плато (TERR-5, TERR-7)', () => {
