...
- Под капотом legacy `sendP2PPacket` всё равно авто-релеится через SDR — то есть для **listen-режима «доверенным сторонам»** даже `ceifa/steamworks.js` технически скрывает IP хоста и проходит NAT. Это делает его приемлемым для listen-режима на текущем этапе.
...
1. **Текущий этап:** остаёмся на WebSocket (wss) — уже есть, TLS «из коробки», достаточно для интерполяции без prediction.
...
**Этап 1 (первый запуск):** WebSocket (wss) + process-per-match + 1 control-plane VPS + пул Hetzner (EU: Falkenstein/Helsinki). Steam-лобби как rendezvous для listen. Без auth для listen-«доверенным». Реализуйте reconnect-окно и rate-limiting на границе. **Не строить** свой UDP, SDR, GameLift, Agones на этом этапе.
