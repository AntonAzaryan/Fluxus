...
7. **Кто получает `viewpoint = ALL`** (netcode). Для локального прогона решено: полный поток идёт только соединению, объявленному наблюдателем в конфиге матча, и игровому слоту не выдаётся ни при каких условиях — номер команды слота проверяется на диапазон `[0, 31]`, чтобы `VIEWPOINT_ALL` не въехал опечаткой в конфиге (`netcode-transport` NTR-9). Открытой остаётся продакшн-авторизация: сетевой аутентификации в MVP нет, и матч в таком виде непригоден для игры с недоверенными участниками.
