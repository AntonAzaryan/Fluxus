...
| 28 | Одна сцена дуэли: паритет оболочек | 🟡 `unify-duel-scene`. Матч включает физику и регистрирует те же системы, что демо; две сцены сводятся в один документ — выполнение CONT-3, невыполненного этапом 27. Поведение матча меняется по существу: `worldInitHash` другой, эталоны матча переписываются |
| 29 | Fixed-тригонометрия таблицей | 🔜 `sin`/`cos` в Math API (FP-7+). Отдельная capability-работа, а не довесок: таблица (или формула её порождения с правилом округления) становится нормативными данными наравне с раскладкой карты пола (TERR-6) — от неё зависит побитовая парность с Rust-портом (CLI-6). Требует решить единицу угла: binary angle measure (полный оборот = 2^16) даёт бесплатное заворачивание, радианы в Q16.16 теряют точность |
