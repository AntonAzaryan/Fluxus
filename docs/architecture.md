...
└───────────────▲──────────────────────────────────┘
        ...
        ┌───────┴────────────────┐
        │  External Observers     │  ← аналитика / логи / звук / рендер и камера
...
| 32 | Камера изометрической арены | ✅ `engine/render-ts/src/camera/` (`camera` CAM-1..7, `assets` ASSET-8): конвейер «режимы → логическая поза → стек эффектов → финальная поза», follow/free-RTS/fly по Dota-семантике, зум, маршрутизация ввода (камере — своё, в `InputFrame` не течёт), эффекты по манифесту визуалов. Конвейер общий игровому клиенту и вьюпорту редактора: follow-цель nullable, поверхность и границы инжектируются, эффекты опциональны (change `camera-system`, стык с редактором — `editor-camera-seam`) |
