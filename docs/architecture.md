...
└───────────────▲──────────────────────────────────┘
        ...
        └───────▲─────────────────┘
                │ зависимость строго односторонняя (DI-3, NTR-1)
┌───────────────┴──────────────────────────────────┐
...
| `editor` | Редактор сцен и геймплея на стеке движка, веб и десктоп: вьюпорт, расстановка, кисти, триггеры, превью | ED-1..19 |
...
| `rendering` | Рендер-наблюдатель: интерполяция, модели, анимации, кости, скины, террейн, кривизна и наклон, подсистемы | REND-1..10 |
| `camera` | Клиентская камера: режимы follow/free-RTS/fly, зум, поза как данные, отношение к rewind, слой эффектов | CAM-1..6 |
...
| 12 | Editor MVP | ⏸ Отложен (решение 2026-08-02): редактор и его рендер спекаются отдельно, ядро доводится до конца раньше. Стек переустановлен change'ом `retarget-editor-to-web-desktop` (2026-08-06): редактор на TypeScript поверх пакетов движка, вьюпорт — `render-ts`, превью — оболочка `client-ts`; запускается и в вебе, и на десктопе из одной кодовой базы. Спека редактора сцен готова change'ем `scene-editor` (2026-08-06): вьюпорт, расстановка, выделение, undo/redo, таксономия unit/prop/decoration, просмотрщик ассетов, обязательное превью (ED-13..19); движковые блокеры реализации записаны стабами `terrain-texturing`, `viewport-services`, `presentation-scene-layer` |
...
| 30 | Каст фаербола в JSON | 🔜 Снимает первый из двух блокеров data-driven геймплея. `aimDir` — единственный скаляр в `InputFrame` (TICK-2), тригонометрии нет, поэтому демо пакует направление в i32 и распаковывает сдвигами, которых в DSL нет. После этапа 29 `aimDir` читается как угол и разворачивается в вектор прямо в JSON; всё остальное касту уже доступно (`bitTest`, `vec.normalize`, `vec.scale`, `spawnEntity` с переопределением, `emitEvent`). Альтернатива без этапа 29 — передавать направление вектором (`aimX`/`aimY`), но это правка TICK-2, протокола и эталонов |
| 31 | Разрушение пола действием ядра | 🔜 Снимает второй блокер. Сегодня выбивание бита пола — нативная система демо: адресация слова `w000…` и битовая арифметика в DSL не выражаются и не должны. TERR-6 сценарий «взрыв выбивает пол» уже нормирован, механизма нет — нужно первоклассное действие (снятие пола по позиции с радиусом) и запрос `hasFloorAt` в DSL. Политика — что выбивает пол, каким радиусом, какой ценой — остаётся в JSON |

Этапы 28–31 — последовательность, а не альтернативы: 28 убивает дублирование документов, 30 и 31 переводят драфтовые нативные системы демо в JSON, после чего нативными остаются только `InputSystem` и `PhysicsSystem` — механизм движка, которому в JSON не место (TICK-4, DI-3). Этап 29 самостоятелен и стоит перед 30 потому, что делает его дешёвым: с тригонометрией `aimDir` остаётся тем, чем назван, и нормативный `InputFrame` не трогается вовсе.
